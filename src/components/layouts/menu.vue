<template>
  <div
    class="la-menu"
    :class="{ 'la-menu-scaled': scaled }"
    v-if="path.indexOf('/login') == -1"
  >
    <el-scrollbar>
      <!-- 搜索部分 -->
      ...
      <div class="menu-item" id="search-res">
        ...
        <div class="submenu">
            ...
            <ul class="group-list">
              <li v-for="list in lists.group">
                ...
                >
                  ...
                  <span
                    v-html="colorKeyword(list.label, input, list.internal)"
          ...
          <div class="menu-nodata" v-if="!resultList.length">
            ...
            <div class="msg">
              对不起，小智暂时没有找到相关数据，<span
                v-if="allList.length === 0 && allNavMapList.length === 0"
                >换个关键词试试吧~</span
              >
            </div>
          </div>
          <span
            v-if="isNavMore"
            class="nav-link"
            :class="{ 'nav-link-center': isNavMore && allList.length === 0 }"
            @click="goNav"
            >去导航地图查看更多结果</span
          >
        </div>
      ...
      <div class="menu-item" v-for="item in menuList" :key="item.label">
        ...
        >
          <div
            v-if="!scaled"
            class="title miniheight"
            :style="
              item.shortlabel == '其他' || item.shortlabel == '导航'
                ? 'cursor: pointer;'
                : ''
            "
            @click.stop="getLabelOne($event)"
          >
            ...
            <i
              ...
              v-if="item.groupList[0].group.length > 0"
            ></i>
          </div>
          <div
            v-else
...
export default {
  ...
  watch: {
    ...
    showPromtNotice: {
      handler(val) {
        document.querySelector(".la-menu").style.paddingTop =
          (val ? 78 : 48) + "px";
      },
    },
  },
  methods: {
    ...
    swicthMenu() {
      if (
        ...
        "home" /* && this.$route.path.split('/')[1] !== 'literature' */
      ) {
    ...
    getSearchResult() {
      ...
      if (value !== "") {
        for (let i = 0; i < length; i++) {
          let getOne =
            mnl[i].groupList instanceof Array
              ? mnl[i].groupList[0]
              : Object.values(mnl[i].groupList)[0];
          let groupBefore = [];
          let getOneGroup = getOne.group;

          for (let item of getOneGroup) {
            if (
              item.label.toUpperCase().indexOf(value) !== -1 ||
              PinyinMatch.match(item.label, value)
            ) {
    ...
    colorKeyword(word, key, isInternal) {
      // 高亮关键词
  ...
  mounted() {
    if (document.querySelector(".la-menu"))
      document.querySelector(".la-menu").style.paddingTop =
        (this.showPromtNotice ? 78 : 48) + "px";
    if (document.querySelector(".la-left-side"))
      document.querySelector(".la-left-side").style.top =
        (this.showPromtNotice ? 78 : 48) + "px";
  },
};
</script>
