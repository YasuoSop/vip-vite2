<template>
  <div style="display: inline-block">
    ...
    <!-- 导出 - 框体 -->
    <el-dialog class="dialog-actions" ref="exportDialog" :visible.sync="exportVisible" :modal-append-to-body="false">
      <div slot="title" class="dialog-header">
        导出
      </div>
      <div>
        <p>当前数据量为<span class="cl-red">{{max_num}}</span>条，最大导出量为 <span class="cl-gray1"><span class="cl-red">{{out_max ? out_max : 300}}</span>条是否导出？</span></p>
        <div><b>导出字段</b></div>
        <div class="offset-top"><el-checkbox :indeterminate="isIndeterminat" v-model="checkallExport" @change="handleCheckAllExport">全选</el-checkbox></div>
...
export default {
  ...
  props: {
    vuex_name: { // 例如：注册数据库为Zhuce
      default: '',
      type: String,
    },
    api_name: { // 例如：注册数据库为zhuce
      default: '',
      type: String,
    },
    max_num: { // 当前数据量，例如：注册数据库中按受理号查询为extendCount.BaseCount
      default: 0,
      type: Number,
    },
    out_max: { // 最大导出条数，默认为300
      default: 300,
      type: Number,
    },
    defaultFiled: {
  ...
  methods: {
    setScrollHeight(){
      this.$nextTick(()=>{
          let el = this.$refs.exportDialog.$el.getElementsByClassName('el-scrollbar__wrap')
    ...
    handleActionExport () {
      if (this.nopms && !this.nopms.dc) {
        ...
        if (this.storeState.allBase) {
          window.ga("send","event", "button", "click", this.storeState.allBase.dbename+"_export")
          window._paq.push(['trackEvent', 'button', 'click', this.storeState.allBase.dbename+"_export", ])
        }else if(this.vuex_name == "Database" ){
          const {params} = this.$route;
          window.ga("send","event", "button", "click", params.dbname+"_export");
          window._paq.push(['trackEvent', 'button', 'click', params.dbname+"_export", ])
    ...
    handleExport () {
      ...
      _.map(this.exportDefaultFiled, item => {
        for (let pt of this.exportArr) {
          if (pt == (item.prop ? item.prop : item.field)) {
            column.push((item.prop ? item.prop : item.field) + "=" + item.label)
            break
          }
        }
      })
      ...
      let urlEncode = url +
        Qs.stringify((this.storeState.gaoji && this.storeState.gaoji.words) ? this.storeState.gaoji : {}) + '&' +
        Qs.stringify(query)
      window.open(
        decodeURI(urlEncode)
      )
