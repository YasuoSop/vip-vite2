<template>
  <div class="la-optioned clearfix" :class="{'option-min-height':minHeight, 'option-show': Object.keys(storeState.putong).length || (storeState.gaoji && storeState.gaoji.words) || (storeState.fourth && storeState.fourth.fourth_condition) || (storeState.hot && storeState.hot.hot_condition) || (storeState.shaixuan && storeState.shaixuan.filter_condition) || (storeState.filtersSource && storeState.filtersSource.filter(item=> item.checked!==0).length>0)}">
    ...
    v-if="Object.keys(storeState.putong).length || (storeState.gaoji && storeState.gaoji.words) || (storeState.fourth && storeState.fourth.fourth_condition) || (storeState.hot && storeState.hot.hot_condition) || (storeState.shaixuan && storeState.shaixuan.filter_condition) || (storeState.filtersSource && storeState.filtersSource.filter(item=> item.checked!==0).length>0)">
      ...
      <!-- 普通搜索 -->
      <span
        v-for="(option, key, index) in (storeState.putong ? handlePutong(storeState.putong) : {})"
        :key="index"
        ...
        <span class="btn option">
          ...
          <span class="tj">{{getPutongCnameLeft(key)+' : '+ handleCutLength(getPutongCnameRight(option,key))}}</span>
          <span class="el-icon-close" @click="handlePutongCloseClick(key)"></span>
        </span>
      </span>
      <!-- 高级搜索 -->
      <span
        v-for="(option, index) in (storeState.conditions ? storeState.conditions : [])"
        :key="index"
        v-if="option.value?option.value instanceof String ? Boolean(option.value) : Boolean(option.value.length):false"
        ...
        <span class="btn option">
          ...
          <span class="tj">{{getGaojiCnameLeft(option.name, option.accurate, option.atcs_sel)+' : '+ handleCutLength(getGaojiCnameRight( option.value, option.name, option))}}</span>
          <span class="el-icon-close" @click="handleGaojiCloseClick(option)"></span>
        </span>
      </span>
      <!-- 热点筛选 -->
      <div class="optioned screen" v-if="(storeState.hot && storeState.hot.hot_condition && getHot(storeState.hot.hot_condition))">
        <!-- <span class="label iconfont">&#xe647;</span> -->
        <span
          class="show-options hot-option"
          :title="getHot((storeState.hot.hot_condition ? storeState.hot.hot_condition : ''))">
          <span class="btn option">
            <!-- ' 热点搜索: '+ -->
            <span class="tj"> {{' 热点搜索: '+handleCutLength(getHot(storeState.hot.hot_condition))}}</span>
            <span class="el-icon-close" @click="handleHotCloseClick(option)"></span>
          </span>
        </span>
      </div>
      ...
      <div class="optioned screen" v-if="(storeState.shaixuan && storeState.shaixuan.filter_condition)">
        ...
        <span
          class="btn option"
          v-for="(option, index) in getShaixuan((storeState.shaixuan.filter_condition ? storeState.shaixuan.filter_condition : ''))"
          :key="index"
          :title="option.value">
          <!-- getShaixuanCnameLeft(option.name)+' : '+ -->
          <span class="tj">{{getShaixuanCnameLeft(option.name)+' : '+ handleCutLength(option.value)}}</span>
...
<script>
  ...
  export default {
    props: {
      ...
      minHeight: { //列表有tabs为false，列表无tabs为true
        type: Boolean,
      }
    },
    components: {
    ...
    methods: {
      ...
      handlePutong (options) {
        ...
        // 除综合搜索外的其他普通搜索
        for (let key in options) {
          if (key != "comprehensive" && key != "searchwords" && key != "atcs_sel") {
      ...
      getPutongCnameRight(option,key) {
        ...
        if(key.indexOf('_eq')) {
          key =  key.replace('_eq','')
				...
				let subQuery = this.storeState.queryStr? JSON.parse(this.storeState.queryStr): '';
        let isSel
        for (let i = 0; i < subQuery.length; i++) {
          ...
          if (item.value == option && item.selectLabel) {
            ...
            isSel = true
            option = '是'
          }else if(key === item.name && item.type === 'fullselect' && item.name !== 'atc') {
            isSel = true
      ...
      getGaojiCnameRight (option, name, obj) {
        ...
        if (_.isArray(option)) { // select组件
          ...
          let str = arr.join('，')
          return str
        } else if (obj.type === "fullselect") { // 多级下拉组件
          // 将value值转换为数组，便于后续操作
          option = option.split('⊙')
          let filters_val = []
            ...
            if(Object.keys(this.storeState.match_shaixuan_values).length && name!=='occr_country' && name!=='dailiangcaigou1' && name!=='leixingbm' && name !=='syzbm'){
              ...
              for (let key in this.storeState.filters) {
                if (name == key) {
                  ...
                  // }
                  // 调用获取条件筛选中文名的方法
                  this.getFliterZh(this.storeState.filters[key],value_arr,filters_val,index)
                }
              }
            }
          }
      ...
      getHot (h_condition) {
        ...
        for (let i = 0, l = arr.length; i < l ; i++) {
          ...
          let value = arr[i].split("=")[1]
          let value_arr = value.split("ღ")
          let fileds = this.storeState.hot_filter_filed[key]
      ...
      handlePutong1(option){
        _.remove(this.storeState.query, item => {
          ...
          if (option && item.name === option.split('_')[0] ) {
            if ((item.name === 'comprehensive'|| item.name === 'searchwords') && (option === 'comprehensive_drug' || 'comprehensive_company' || 'comprehensive_devices' || 'comprehensive_chinesemedicine' || 'comprehensive_targets' || 'comprehensive_disease')) {
      ...
      handleFourthCloseClick () {
        ...
        if (this.route) {
          if (this.route == this.route1) {
            ...
            console.log("analy: ",this.vuex_name + '/getKshRes')
            Store.dispatch(this.vuex_name + '/getKshRes')
          }
        } else {
