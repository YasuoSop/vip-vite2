<template>
  <div>
    ...
    <!-- 左侧联级选择组件（atc专用） -->
    <LeftCascaderSelect v-if="item.name == 'atc' && item.type=='fullselect'" :vuex_name="vuex_name" :item="item"></LeftCascaderSelect>
    <!-- 左侧联级选择组件（非atc） -->
    <LeftFullselect v-if="item.name != 'atc' && item.type=='fullselect'" :vuex_name="vuex_name" :item="item"></LeftFullselect>
    <!-- 左侧选择组件 -->
...
import LeftFullselect from '@/components/inputs/left-fullselect.vue'
import LeftSelect from '@/components/inputs/left-select.vue'
import LeftCheckBox from '@/components/inputs/LeftCheckBox.vue'
