...
<!-- 搜索左侧 热点搜索 -->
  <transition name="el-zoom-in-top" appear>
    <div class="hot-filter item" v-if="hasHotFilters" :class="{'abandon-click-method': nopms && nopms.rdss}">
        ...
        <div class="filter-lists_box">
          <div v-for="(key, index) in keys" :key="index">
            <el-checkbox-group v-model="checked[key]">
              <el-checkbox :disabled="nopms && nopms.rdss" v-for="item in hot_filed[key]" :label="item.key" :key="item.key" @change="handleCheckedChange(item.key, item.param)">
                <span>
                  <!-- {{item.label}} -->
                  <NewProjectGuide  postion="热点" :onlyKey="item['new_id']"  display="inline" compoentsName="条件筛选" :initItem="item" :showMenban="item['is_show_new']">{{item.label}}</NewProjectGuide>
                </span>
      ...
      <el-button class="control-filter" :disabled="nopms && nopms.rdss" size="mini" type="primary" @click="search">筛选</el-button>
    </div>
  </transition>
</template>
