  ...
  <div class="la-advanced">
    <table class="la-table">
      ...
      <tr v-for="(condition, tr_index) in conditions" :key="tr_index">
        ...
        <!-- 2、类型选择 -->
        <td class="label" :class="{'atc-label':condition.name=='atc'||condition.name=='atcb'}">
          <Select v-model="condition.name" @change="typeChange(tr_index, condition)" placeholder="请选择类型">
            <el-option
              ...
              :value="label.name">
                ...
                <img src="/static/imgs/newProjectGuide/new.png" v-if="label['is_show_new']" alt="" />
              ...
              <el-tooltip v-if="label.statement"
                effect="light"
                placement="right"
                >
              ...
              <el-tooltip v-else-if="label.label=='药理分类' && vuex_name == 'Yaopinxiaoshou'"
                effect="light"
                placement="right"
                content="ATC编码分类">
                <i class="el-icon-question cl-green"></i>
          ...
          <el-tooltip v-if="condition.name=='atc'||condition.name=='atcb'"
            effect="light"
            placement="right"
          ...
          <NewProjectGuide v-if="condition['is_show_new']" postion="高级"
          :onlyKey="condition['new_id']"
          :newPosition="{top:-5,right:12.5-condition.label.length}"
          compoentsName="检索功能"
          :initItem="condition"
          Ldanwei='em'
          :showMenban="condition['is_show_new']||''"><span> </span></NewProjectGuide>
           <!-- <NewProjectGuide :onlyKey="'c_16044688066666'+tr_index" display="inline" :initItem="''"></NewProjectGuide> -->
        </td>
        <!-- 3、输入框/选择框/日期选择框 -->
        <td class="value">
          <!-- 高级搜索项 -->
          <template v-for="(item, td_index) in advancedfileds" v-if="item.name == condition.name">
            <!-- 输入框 - 含搜索提示 -->
            <AutoInput
            ...
            <DateRangeMonth
              ...
              @updateValue="updateValue"
              :key="td_index"
              ref="iadvanced"></DateRangeMonth>
            <Select
              v-if="item.type=='select'"
              :option="item.value" :valueList="condition.value ? condition.value.split ? condition.value.split('ღ') : condition.value : []"
              :index="tr_index"
              @updateValue="updateValue"
              :key="td_index"
              multiple
              collapseTags
              ref="iadvanced"></Select>
            <CascaderSelect
              v-if="checkNameBeginWithAtc(item.name) && item.type=='fullselect'"
              :vuex_name="vuex_name"
              :item="item"
            ...
            <FullSelect
              ...
              @updateValue="updateValue"
              :key="td_index"
              ref="iadvanced"></FullSelect>
          </template>
        </td>
        <!-- 4、添加/删除条件的操作按钮（+/-） -->
        <td class="ctrl">
          <i v-if="tr_index != 0" class="btn btn-advanced-remove el-icon-minus" @click="removeItem(tr_index)"></i>
          <i class="btn btn-advanced-add el-icon-plus" @click="addItem(tr_index)"></i>
        </td>
      </tr>
    </table>
    <!-- 高级搜索框底部 -->
    <div slot="footer" class="dialog-footer">
       <el-button @click="advancedSearch" type="primary" round class="ad-search">搜 索</el-button>
...
<script>
  import NormalInput from '@/components/inputs/input.vue'
  import AutoInput from '@/components/inputs/auto-input.vue'
  import Select from '@/components/inputs/select.vue'
  import DateRange from '@/components/inputs/date-range.vue'
  import DateRangeYear from '@/components/inputs/date-range-year.vue'
  import DateRangeMonth from '@/components/inputs/date-range-month.vue'
  import CascaderSelect from '@/components/inputs/cascader-select.vue'
  import NumberRange from '@/components/inputs/number-range.vue'
  import FullSelect from '@/components/inputs/fullselect.vue'
  ...
  export default {
    ...
    computed: {
      advancedfileds () {
        return Store.state[this.vuex_name].advanced_filed.map(item=>{
          if(item.type==='select'&&item.accurate==='精确'&&item.name.indexOf('_eq')===-1){
    ...
    watch: {
      dialogFormVisible(n){
        if(n){
          window.ga("send", "event", "botton", "click", this.api_name+"_advanced_search_outside");
          window._paq.push(['trackEvent', 'button', 'click', this.api_name+"_advanced_search_outside", ])
    ...
    methods: {
      ...
      initConditions () {
        ...
        if (labelslen < this.advancedLen) { // 如果初始规定的高级搜索显示条数多于高级搜索总类数，则使之相等
        ...
        if (querylen != 0 && !queryHasSearch&&this.query.every(item=>item.type!=='check')) { // 普通填写 & 无论高级是否填写（且不属于综合搜索跳转过来的情况下！）
          this.conditions = []
        ...
        } else if (!advancedIsClear||!conditionsIsClear&&querylen == 0 && conditionslen != 0 && !advancedIsClear) { // 高级搜索还有条件在里面
          this.conditions = []
        ...
        if(!this.conditions.length){
          this.frontEndLog(location.href, {
            handlerTypeDesc: '高级搜索弹框数据记录（点击 高级搜索，没有显示数据（偶然））',
            res: {
      ...
      advancedSearch () {
        ...
        window._paq.push(['trackEvent', 'button', 'click', this.api_name+"_advanced_search_outside", ])
        window.ga("send", "event", "botton", "click", this.api_name+"_advanced_search_outside")
        window._paq.push(['trackEvent', 'button', 'click', this.api_name+"_advanced_search_inside", ])
        window.ga("send", "event", "botton", "click", this.api_name+"_advanced_search_inside")
      },
