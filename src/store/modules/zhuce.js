...
export default {
  ...
  actions: {
    ...
    nomalSearch (state, Payload) {//type为1时是为了
      ...
      let zhuangtai = [],guifanzhuangtaizhongwen=[],zhuangtaiStr='',guifanzhuangtaizhongwenStr=''
      ...
      if (zhuangtai.length>0) {
        zhuangtaiStr = ` _and zhuangtai=${_.join(zhuangtai, 'ღ')}`
      }
      // 当guifanzhuangtaizhongwen筛选项大于0时，_and拼接
      if (guifanzhuangtaizhongwen.length>0) {
        guifanzhuangtaizhongwenStr = ` _and guifanzhuangtaizhongwen=${_.join(guifanzhuangtaizhongwen, 'ღ')}`
      }
      console.log(guifanzhuangtaizhongwenStr,zhuangtaiStr);

      if (sgroup.filter_condition) {
        sgroup.filter_condition = sgroup.filter_condition + zhuangtaiStr + guifanzhuangtaizhongwenStr
      } else {
        sgroup.filter_condition = zhuangtaiStr + guifanzhuangtaizhongwenStr
      }
      ...
      return window.Axios({
        ...
        if (res.data.data) {
          ...
          //二级列表数据关联到一级列表数据
          let addSecData = state.state.tabname!='first' && data.List ? data.List.map((item)=>{
            item.second = {}
            return item
          }) : []
    ...
    getKshRes (state) {
      ...
      this.vue.getSearchParams('zhuce',state,param,'visualization')
      return window.Axios({
        method: 'get',
        url: '/api/zhuce/visualization',
        params: _.assign(param, {
          accesstoken: GETCOOKIEFUN("accesstoken")
        })
      }).then(res => {
