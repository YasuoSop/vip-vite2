...
export default {
    ...
    mutations: {
        ...
        tableconf(state, Payload) {
            for (let item of Payload) {
                if (item.hidden) {
                    item = _.assign(item, { 'checked': false })
                } else {
                    item = _.assign(item, { 'checked': true })
                }
            }
        ...
        nomalSearch(state, Payload) {
            ...
            if (state.state.database_name === 'biaozhundb' && Object.keys(param).filter(key => key !== 'page' && key !== 'pageSize' && key !== 'order').length) {
                if (!param.order) {
                    ...
                    state.commit("param", Object.assign({}, state.state.param, { order: 'wenjianming desc' }))
                }
            } else if (state.state.database_name === 'biaozhundb' && state.state.param.order) {
                state.commit("param", Object.assign({}, state.state.param, { order: null }))
            }
