...
export default {
  ...
  actions: {
    ...
    nomalSearch (state, Payload) {//type为1时是为了
      ...
      this.vue.getSearchParams('fdadrugnew',state,param)
      return window.Axios({
        method: 'get',
        ...
        if (res.data.data) {
          ...
          //二级列表数据关联到一级列表数据
          let addSecData = state.state.tabname!='basegroup' && data.List.res ? data.List.res.map((item)=>{
            item.second = {}
            return item
          }) : []
    ...
    getKshResGroup (state, Payload) {
      ...
      if (state.state.gaoji.words) {
        ...
        param = (Payload && Payload.params) ? _.assign({}, Payload.params, sfourth, swords, sgroup,shot) : _.assign({}, sfourth, swords, sgroup,shot)
      } else {
        // 普通搜索条件 条件筛选条件 传过来的附加条件（如果有的话 => page pageSize order） - 进行组合
        param = (Payload && Payload.params) ? _.assign({}, Payload.params, sfourth, sparam, sgroup,shot) : _.assign({}, sfourth, sparam, sgroup,shot)
      }
      //关键词统计方法
      param = _.assign(param,this.vue.searchKeywords(state.state))
      this.vue.getSearchParams('fdadrugnew',state,_.assign({}, param, {count_type: 'base_count'}),'visualization')
