<template>
  <List :loading="listLoading" :nodata="nodata">
    <div style="display:flex;">
      <div id="statemap" :style="{height: tableHeight+'px',width: '70%'}"></div>
      <div class="map-table">
        <el-table
          :data="seriesData"
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span class="underline-span" @click="nextClick({name: scope.row.name}, item.prop)">{{scope.row[item.prop]}}</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
  ...
  export default{
    ...
    methods: {
      ...
      drawMap (mapdata){
        ...
        let statemapId = document.getElementById('statemap')
        ...
        this.myChart.setOption({
          toolbox: {
            ...
            right: 30,
            feature: {
              myToggleMapt: this.mapJsonName !== 'State' ?{
                show: true,
                title: "返回",
                icon: "image:///static/imgs/echarts/check.png",
                onclick() {
                  _this.mapJsonName = 'State'
                  _this.mapData()
                },
              }:{},
              saveAsImage: {
      ...
      nextClick (params, pop) {
        ...
        if(this.mapJsonName==='State'){
          ...
          _this.param.page = 1
          _this.param.pageSize = 100
          _this.param.cate = params.name
          _this.param.type = _this.cate_param_one
          _this.param.order = 'doc_count desc'
          Store.dispatch('Clinical/getCatestand',{params: _.assign({}, _this.param, {is_visual: 1})})
          ...
          _this.vueExtendTo('/clinical?'+_this.guanlian[_this.changeBox][_this.cate_param_one]+'_eq='+params.name)
