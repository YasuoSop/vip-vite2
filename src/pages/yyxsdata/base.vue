<template>
  <div class="yyxsfx">
    ...
    <header class="header" :class="onTrial ? 'onTrial' : ''">
      ...
      <div v-if="onTrial" class="header-bottom" :style="{ margin: onTrial ? '5px 0' : '0' }">您当前为试用用户，仅提供2016，2017，2018年销售数据可查，正式用户可查最新数据</div>
    </header>
      ...
      <div class="la-right">
        <section class="sec2">
          <template v-if="ypfxTnit">
            <!--xxxMagnify表示：放大测试版 -->
            <page1
              v-if="nowNav==='药品分析' && is_magnify == false"
              :ypname="ypname"
              :nopmState="nopmState"
              :type="type"
            />
            <page1Magnify
              v-else-if="nowNav==='药品分析' && is_magnify == true"
              :ypname="ypname"
              :nopmState="nopmState"
              :type="type"
            />
          ...
          <template v-if="qyfxInit">
            <page2
              v-if="nowNav==='企业分析' && is_magnify == false"
              :qyname="qyname"
              :nopmState="nopmState"
            />
            <page2Magnify
              v-else-if="nowNav==='企业分析' && is_magnify == true"
              :qyname="qyname"
              :nopmState="nopmState"
            />
          </template>
          <template v-if="flfxInit">
            <page3
              v-if="nowNav==='分类分析' && is_magnify == false"
              :qyname="qyname"
              :nopmState="nopmState"
            />
            <page3Magnify
              v-else-if="nowNav==='分类分析' && is_magnify == true"
              :qyname="qyname"
              :nopmState="nopmState"
            />
          </template>
...
export default {
  ...
  mounted() {
    if (this.tab) {
      ...
      console.log(localStorage.nowNav);
