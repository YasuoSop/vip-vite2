    ...
    <div v-if="edata.curType == 'table'" class="echart-table-wrap">
      <div class="export-table" v-if="!noexport && edata.data">
        更多数据点击<span
          v-if="nopmState"
          @click="clickeExport"
          :class="{ 'abandon-click-method': hasPermission }"
...
export default {
  ...
  methods: {
    ...
    clickeExport() {
      ...
      let fileName = this.multiple ? this.qyTitle : this.edata.title;
      if (
        ...
        url = `/api/yyxs/${this.edata.url}?${yearStr}&${this.queryName}=${this.name}&${typeStr}&${qyStr}&outdata_column=${headStr}tablename=${fileName}&accesstoken=${token}`;
          ...
          tooltip: {
            ...
            formatter: list => {
              ...
              if (is_magnify) {
                ...
                str = `${list[0].name == "undefined" ? "" : list[0].name}${
                  this.year ? this.year : ""
                }&nbsp;&nbsp;&nbsp;&nbsp;${zeStr}<br/>`;
              }
              _.forEach(list, item => {
