      ...
      <div class="chart_top">
        <div class="newtotals">
          <ul class="total-item">
            ...
            <li class="li2">总计<span>{{aggs.me_id ? aggs.me_id : 0}}</span>个</li>
            <li class="li3">3期临床<span class="num-s">{{aggs.phase ? aggs.phase : 0}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">申办者单位</li>
            <li class="li2">总计<span>{{aggs.qiyemingcheng ? aggs.qiyemingcheng : 0}}</span>个</li>
            <li class="li3" v-if="aggs.most_qiyemingcheng&&aggs.most_qiyemingcheng.count">{{aggs.most_qiyemingcheng.name}}最多<p>共<span class="num-s">{{aggs.most_qiyemingcheng.count}}</span>个</p></li>
          </ul>
          <ul class="total-item">
            <li class="li1">药品</li>
            <li class="li2">总计<span>{{aggs.name_guifan ? aggs.name_guifan : 0}}</span>个</li>
            <li class="li3" v-if="aggs.most_type&&aggs.most_type.count">{{aggs.most_type.name}}最多<p>共<span class="num-s">{{aggs.most_type.count}}</span>个</p></li>
          </ul>
          <ul class="total-item">
            <li class="li1">试验机构</li>
            <li class="li2">总计<span>{{aggs.institutes_guifan ? aggs.institutes_guifan : 0}}</span>个</li>
            <li class="li3" v-if="aggs.most_institutes_guifan&&aggs.most_institutes_guifan.count">{{aggs.most_institutes_guifan.name}}最多<p>共<span class="num-s">{{aggs.most_institutes_guifan.count}}</span>个</p></li>
          </ul>
          <ul class="total-item">
            <li class="li1">已完成</li>
            <li class="li2">总计<span>{{aggs.me_status ? aggs.me_status : 0}}</span>个</li>
            <li class="li3">3期临床完成<span class="num-s">{{aggs.me_status_phase ? aggs.me_status_phase : 0}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">一致性评价品种</li>
            <li class="li2">总计<span>{{aggs.yzx ? aggs.yzx : 0}}</span>个</li>
          </ul>
        </div>
      </div>
      <div class="chart_bottom">
        <div class="go-list">
          <router-link tag="a" :to="{path:'/linchuangshiyan/list.vue'}">返回列表</router-link>
        </div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      aggs: state => state.Linchuangshiyan.kshRes.aggs ? state.Linchuangshiyan.kshRes.aggs : {},
      view: state => state.Linchuangshiyan.kshRes.view ? state.Linchuangshiyan.kshRes.view : {},
  ...
  methods: {
    ...
    update() {
      let {views_djqs,views_sbzdw,views_syjg,views_ypph,views_yplx,views_syzt,views_syfl,views_syfq} = this.view
  ...
  watch: {
    ...
    $route(val){
      ...
      if(this.$route.path==='/linchuangshiyan/list.vue'){
