          ...
          <div class="doc" v-for="(oneDoc, idx) in res1" :key="idx">
            ...
            <div class="head">
              <div class="head-msg">
                ...
                <router-link :to="{path: '/doc/' + oneDoc.id, query: {tb: oneDoc.tb}}" class="name" target="_blank">{{oneDoc.type}} <span v-if="oneDoc.typezh">({{oneDoc.typezh}})</span></router-link>
              </div>
              <div class="head-sub">
                <template v-if="oneDoc.tb == 3">
                  <span class="name-cname" v-if="oneDoc.name"><i class="iconfont">&#xe650;</i>药品名称：{{oneDoc.name}}</span>
                </template>
                <template v-else>
                  <span class="name-cname" v-if="oneDoc.name&&oneDoc.cname"><i class="iconfont">&#xe650;</i>药品名称：{{oneDoc.name}}[{{oneDoc.cname}}]</span>
                </template>
                <span class="sponsor" v-if="oneDoc.sponsor"><i class="iconfont">&#xe651;</i>申请方：{{oneDoc.sponsor}}</span>
                <span class="date" v-if="oneDoc.date"><i class="iconfont">&#xe675;</i>发布日期：{{oneDoc.date}}</span>
              </div>
            </div>
            <div class="main-con">
              <router-link :to="{path: '/doc/' + oneDoc.id, query: {tb: oneDoc.tb}}" class="content" v-html="oneDoc.txt" target="_blank"></router-link>
            </div>
            <div class="list-bottom">
              <span class="type" v-if="oneDoc.tb">{{ oneDoc.tb | tbFilter }}</span>
              <span class="type" v-if="oneDoc.tag" v-for="(tag, idxt) in getTagsArr(oneDoc.tag)" :key="idxt">{{tag}}</span>
