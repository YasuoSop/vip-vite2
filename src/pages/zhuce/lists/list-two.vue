    ...
    <List :loading="listLoading" :nodata="res2 && !res2.length">
      ...
      <el-table
                ...
                <el-table-column
                  ...
                  <template slot-scope="scope">
                    ...
                    <div v-if="item.prop === 'shoulihao'">
                      <router-link tag="a" :to="{path:'/zhuce/' + scope.row.id}" target="_blank" class="cl-blue">
                        {{ scope.row.shoulihao }}
                      </router-link>
                    </div>
                    ...
                    <div v-if="item.prop === 'zhuangtaidate'">
                      {{ scope.row.zhuangtaidate }}
                    </div>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div :title="item.prop==='zgzt'||item.prop==='zxzt'?'':scope.row[item.prop]">
              <div v-if="item.prop === 'guifanname'" :class="{'extensible-td': Number(scope.row.guifanname)}">
              ...
              <div v-else-if="item.prop==='yaopin_high'"
              ...
              <div v-else-if="item.prop==='zhuangtaidate'"
              :class="{'extensible-num-td': Number(scope.row.zhuangtaidate.count)}"
              @click="otherGetExpand(scope.row,  item.prop, item.prop,item.prop,scope)"
              :title="scope.row.zhuangtaidate.zhuangtaidate">
                <span v-if="scope.row.zhuangtaidate.zhuangtaidate">
                  {{scope.row.zhuangtaidate.zhuangtaidate}}
                  <span>
                    (<span class="num-text">{{scope.row.zhuangtaidate.count}}</span>)
                  </span>
                </span>
                <span v-else>/</span>
              </div>
              <div v-else-if="item.prop==='company_max'"
              :class="{'extensible-num-td': Number(scope.row.company_max.count)}"
              @click="otherGetExpand(scope.row, item.prop, 'company','guifanqiyezhongbiao',scope)"
              :title="scope.row.company_max.company">
                <span v-if="scope.row.company_max.company">
                  {{scope.row.company_max.company}}
                  <span>
                  (<span class="num-text">{{scope.row.company_max.count}}</span>)
                  </span>
                </span>
                <span v-else>/</span>
              </div>
              <div v-else :class="{'extensible-td': Number(scope.row[item.prop])}" @click="getExpandRes(scope.row.guifanname, item.prop, scope.row[item.prop],scope.row,scope)">
  ...
  export default{
    ...
    computed: {
      ...
      ...mapState({
        ...
        res2: state => _.keys(state.Zhuce.res2).length ? state.Zhuce.res2 : [],
      ...
      currentFiled(){
        let removeFiled = this.otherParams&&this.otherParams.zhuangtaidate
                            ?'最高状态时间'
                            :'最新状态时间'
    ...
    methods: {
      ...
      sortChange (column, prop, order) {
        ...
        Store.commit('Zhuce/order', Object.assign({}, this.$store.state.Zhuce.order, {name: sort ? sort : null}))
        Store.dispatch('Zhuce/nomalSearch',{params:this.param})
      },
      ...
      handleRequest(props, order){
        ...
        Store.dispatch("Zhuce/getGroupRes",{path:'/api/zhuce/namelist',params,row:this.currentRow,type:1}).then(res=>{
      ...
      toggleRowExpansion(row, column, cell, event){
        ...
        let newColum = ['company_max','zhuangtaidate','yaopin_high']
        //前三个字段用表格1
        let tableTypeArr = ['yaopin_high','zhuangtaidate','company_max']
      ...
      handleRss(rsss, type, shoulihao, name, qiyemingcheng) {
        ...
        window._paq.push(['trackEvent', 'button',"click", 'vip_zhuce_yaopin_dingyue',  ])
