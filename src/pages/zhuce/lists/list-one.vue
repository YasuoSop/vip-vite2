    ...
    <List :loading="listLoading" :nodata="res1 && !res1.length">
      ...
      <el-table
        ...
        <el-table-column
          ...
          :prop="item.prop"
          :min-width="item.field_width || 130"
          :key="item.prop"
          :sortable="item.sort"
          ...
          <template slot-scope="scope">
            ...
            <div v-else-if="item.prop === 'name'">
              <div v-if="vueCheckListPms('Zhuce', item.label)">
                <span class="link-underline" @click="resolveTdLink(item.prop, scope.row.name)">{{ scope.row.name }}</span>
            ...
            <div v-else-if="item.prop === 'zhuceleixing'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">
                {{ (scope.row.yaopinleixing_1 ? scope.row.yaopinleixing_1 : '') + (scope.row.zhuceleixing ? scope.row.zhuceleixing : '') }}
              </span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'shenqingleixing_1'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.shenqingleixing_1 }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'zhucebanben'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.zhucebanben }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'chengbanriqi'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.chengbanriqi }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'qiyemingcheng'">
              <span v-if="vueCheckListPms('Zhuce', item.label)" class="link-underline" @click="resolveTdLink(item.prop, scope.row.qiyemingcheng)">{{ scope.row.qiyemingcheng }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
             <div v-else-if="item.prop === 'qiyelianheshenbao'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.qiyelianheshenbao }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'banlizhuangtai'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.banlizhuangtai }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'zhuangtaikaishishijian'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.zhuangtaikaishishijian }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'shenpingrenwufenlei'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.shenpingrenwufenlei }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'xuhao'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.xuhao }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div
              ...
              :class="{'bg-green-br': scope.row.jielun && scope.row.jielun.indexOf('批准') === 0 && vueCheckListPms('Zhuce', item.label),
                'bg-orange-br': scope.row.jielun && scope.row.jielun.indexOf('批准') !== 0 && vueCheckListPms('Zhuce', item.label)}"
              >{{ scope.row.jielun }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
              <!-- 判断类型是否为预测状态：0(预测)、1(预测结论)、2(预测对的结论)、3(有争议的) -->
              <em v-if="scope.row.type == '1' && scope.row.jielun != '' && vueCheckListPms('Zhuce', item.label)" class="status-fore" title="预测结论">预</em>
            </div>
            <div v-else-if="item.prop === 'tspz'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.tspz == 1 ? '是' : '否' }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'zdzx'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.zdzx == 1 ? '是' : '否' }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'yxsp'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.yxsp == 1 || scope.row.yxsp == 4 ? '是' : '否' }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>

            <div v-else-if="item.prop === 'yxsply'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.yxsply }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>

             <div v-else-if="item.prop === 'gdcf'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.gdcf }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
             <div v-else-if="item.prop === 'wftjcbzj'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.wftjcbzj == '是' ? '是' : '否' }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>

            <div v-else-if="item.prop === 'tpxzl'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.tpxzl }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'zjxzsp'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.zjxzsp == 1 ? '是' : '否' }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-else-if="item.prop === 'shengfen'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.shengfen }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
						<div v-else-if="item.prop === 'linchuangdate'">
						  <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.linchuangdate }}</span>
						  <span v-else class="abandon-click-method">暂无权限</span>
						</div>
            <div v-else-if="item.prop === 'shenpingdate'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.shenpingdate }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            ...
            <div v-else-if="item.prop === 'lcjxjwxy'">
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row.lcjxjwxy == 2 ? '是' : '否' }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            ...
            <div v-else-if="item.prop === 'report'">
              <template v-if="scope.row.report != ''">
                <span class="linksBtn" v-if="vueCheckListPms('Zhuce', item.label)" @click="redirectNewPage(scope.row.report)">查看</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </template>
              <span>&nbsp;</span>
            </div>
            ...
            <div v-else-if="item.prop === 'instruct'">
              <template v-if="scope.row.instruct != ''">
                <span class="linksBtn" v-if="vueCheckListPms('Zhuce', item.label)" @click="redirectNewPage(scope.row.instruct)">查看</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </template>
              <span>&nbsp;</span>
            </div>
            ...
            <div v-else-if="item.prop === 'indication2'" :title="''">
              <template v-if="scope.row.indication2 != ''">
                <span v-if="vueCheckListPms('Zhuce', item.label)" v-html="scope.row.indication2"></span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </template>
              <span>&nbsp;</span>
            </div>
            ...
            <div v-else-if="item.prop === 'indication1'" :title="''">
              <template v-if="scope.row.indication1 != ''">
                <span v-if="vueCheckListPms('Zhuce', item.label)" v-html="scope.row.indication1"></span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </template>
              <span>&nbsp;</span>
            </div>
            ...
            <div v-else>
              <span v-if="vueCheckListPms('Zhuce', item.label)">{{ scope.row[item.prop] }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
          </div>
          </template>
  ...
  export default{
    ...
    computed: {
      ...
      ...mapState({
        ...
        res1: state => _.keys(state.Zhuce.res1).length ? state.Zhuce.res1.res : [],
    ...
    methods: {
      ...
      handleRss(rsss, type, shoulihao, name, qiyemingcheng) {
        window.ga("send", "event", "button", "click","vip_zhuce_shoulihao_dingyue")
        window._paq.push(['trackEvent', 'button',"click","vip_zhuce_shoulihao_dingyue",  ])
