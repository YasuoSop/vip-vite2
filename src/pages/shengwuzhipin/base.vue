<template>
  <Main class="shengwuzhipin" >
    ...
    <el-dialog class="dialog-saved" :visible.sync="savedPopVisible" :modal-append-to-body="false">
      ...
      <SavedPop @closeSavedPop="closeSavedPop" vuex_name="Shengwuzhipin"></SavedPop>
    </el-dialog>
    <!-- 高级搜索弹出框 -->
      ...
      <section class="sec2">
        <!-- <div class="la-body-tab">
          <div class="market_nav">
            <a @click.prevent="navRouteTo(1)" class="nav_one" :class="{'router-link-active': $route.path == '/pqf/list.vue'}">
    ...
    <!-- 定量分析 -->
    <full-screen :nowNav='nowNav' :navs='navs' @changeNowNav="changeNowNav" @close="dialogClose" :titles="'生物制品销售精准分析'">
      <!-- ①药品分析 -->
      <div class="fs-page ypfx" slot="检品分析">
        ...
        <div class="totals">
          <ul class="total-item">
            ...
            <li class="li2"><span>{{ypfx.jpgg | formatNumber}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">生产企业</li>
            <li class="li2"><span>{{ypfx.scqy | formatNumber}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">签发年份</li>
            <li class="li2"><span>{{ypfx.qfnf | formatNumber}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">销售量（最新年份）</li>
            <li class="li2"><span>{{ypfx.xsl | formatNumber}}</span></li>
          </ul>
          <ul class="total-item">
            <li class="li1">销售总额（最新年份）</li>
            <li class="li2"><span>{{ypfx.xsze | formatNumber}}</span>万元</li>
          </ul>
        </div>
        <div class="column">
          <div class="view">
            ...
            <el-button v-if="searchValYp" class="top-btn" type="primary" size="mini" round @click="clickBtn"><i class="iconfont download-icon">&#xe62c;</i>下载数据</el-button>
          </div>
        </div>
        <!-- 按企业分析 -->
        <div class="column">
        ...
        <div class="column">
          ...
          <!-- VIEW -->
          <div class="view ndxsfx">
            <div v-if="this.ypfx.val2 == 'all'" class="chart resizeChart" ref="ypfxChart5" key='ypfxChart5'></div>
            <template v-else>
              <div class="chart2 resizeChart" ref="ypfxChart6" key='ypfxChart6'></div>
              <div class="chart2 chartSwitch">
      ...
      <div class="fs-page qyfx" slot="企业分析">
        ...
        <div class="totals">
          <ul class="total-item">
            ...
            <li class="li2"><span>{{qyfx.jpsl | formatNumber}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">签发年份</li>
            <li class="li2"><span>{{qyfx.qfnf | formatNumber}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">销售量（最新年份）</li>
            <li class="li2"><span>{{qyfx.xsl | formatNumber}}</span></li>
          </ul>
          <ul class="total-item">
            <li class="li1">销售总额（最新年份）</li>
            <li class="li2"><span>{{qyfx.xsze | formatNumber}}</span>万元</li>
          </ul>
        </div>
        <div class="column">
          <div class="view">
            ...
            <el-button v-if="searchDefValQy" class="top-btn" type="primary" size="mini" round @click="clickBtn"><i class="iconfont download-icon">&#xe62c;</i>下载数据</el-button>
          </div>
        </div>
        <!-- 按检品分析 -->
        <div class="column">
        ...
        <div class="column">
          ...
          <!-- VIEW -->
          <div class="view ndxsfx">
            <div v-if="this.qyfx.val2 == 'all'" class="chart resizeChart" ref="qyfxChart4" key='qyfxChart4'></div>
            <template v-else>
              <div class="chart2 resizeChart" ref="qyfxChart5" key='qyfxChart5'></div>
              <div class="chart2 chartSwitch">
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      navRouteTo (index) {
        if (index == 1) {
          this.$router.push({ path: '/pqf/list.vue'})
      ...
      showAnaly () {
        ...
        this.$store.commit('Shengwuzhipin/dlfx_searchListYp', yptop.reduce((total, curr) => {
          return total.push({
            label: curr,
            value: curr,
          }) && total;
      ...
      changeNowNav (name) {
        ...
        switch (name) {
          case '检品分析':
            tmp = this.$store.state.Shengwuzhipin.yptop.slice().reverse();
            if (tmp.length) {
              this.$store.commit('Shengwuzhipin/dlfx_searchValYp', tmp[0]);
              this.$store.commit('Shengwuzhipin/dlfx_searchListYp', tmp.reduce((total, curr) => {
                return total.push({
                  label: curr,
                  value: curr,
                }) && total;
          ...
          case '企业分析':
              ...
              if (tmp.length) {
                ...
                this.$store.commit('Shengwuzhipin/dlfx_searchListQy', tmp.reduce((total, curr) => {
                  return total.push({
                    label: curr,
                    value: curr,
                  }) && total;
      ...
      yearyp () {
        return Axios.get('/api/pqf/yearyp', {
          ...
          if (res.data.code == 200 && res.data.data.length) {
            this.ypfx.select2.push(...res.data.data.reduce((total, curr) => {
              return total.push({
                label: curr.dates,
                value: curr.dates,
              }) && total;
            ...
            // 年份-默认值
            this.ypfx.val2 = this.ypfx.select2 && this.ypfx.select2[0].value || '';
          }
        }).catch(err => {
          console.error(err);
        })
      ...
      viewxszl () {
        ...
        return Axios.get('/api/pqf/viewxszl', {
          ...
          if (res.data.code == 200 && res.data.data.length) {
            let t = res.data.data.reduce((total, curr) => {
              return total.x.push(curr.qiye) && total.xl.push(curr.piliang) && total.xse.push(curr.total) && total;
            }, {
              x: [],
              xl: [],
              xse: [],
      ...
      viewggxszl () {
        ...
        return Axios.get('/api/pqf/viewggxszl', {
          ...
          if (res.data.code == 200 && res.data.data.length) {
            let t = res.data.data.reduce((total, curr) => {
              return total.x.push(curr.guige) && total.xl.push(curr.piliang) && total.xse.push(curr.total) && total;
            }, {
              x: [],
              xl: [],
              xse: [],
      ...
      yearqy () {
        return Axios.get('/api/pqf/yearqy', {
          ...
          if (res.data.code == 200 && res.data.data.length) {
            this.qyfx.select2.push(...res.data.data.reduce((total, curr) => {
              return total.push({
                label: curr.dates,
                value: curr.dates,
              }) && total;
            ...
            // 年份-默认值
            this.qyfx.val2 = this.qyfx.select2 && this.qyfx.select2[0].value || '';
          }
        }).catch(err => {
          console.error(err);
        })
      ...
      viewqyxsze () {
        ...
        return Axios.get('/api/pqf/viewqyxsze', {
          ...
          if (res.data.code == 200 && res.data.data.length) {
            let t = res.data.data.reduce((total, curr) => {
              return total.x.push(curr.name) && total.xl.push(curr.piliang) && total.xse.push(curr.total) && total;
            }, {
              x: [],
              xl: [],
              xse: [],
        ...
        return Axios.get('/api/pqf/viewdatexszl', {
          ...
          if (res.data.code == 200 && res.data.data.length) {
            ...
            switch (this.nowNav) {
              case '检品分析':
                if (this.ypfx.val2 == 'all') {
                  t = res.data.data.reduce((total, curr) => {
                    return total.year.push(curr.year) && total.piliang.push(curr.piliang) && total.total.push(curr.total) && total;
                  }, {
                    year: [],
                    piliang: [],
                    total: [],
                  ...
                  t = res.data.data.reduce((total, curr) => {
                    return total.xl.push(curr.piliang) && total.x.push(curr.quarter+'季度') && total.xse.push(curr.total) && total;
                  }, {
                    xl: [],
                    x: [],
                    xse: [],
                  })
                  this.ypfx.chart.qfsj_jd = Object.assign(this.ypfx.chart.qfsj_jd, {
                    legend: ['销量(瓶(支))', '销售额(万元)'],
                    x: t.x,
                    yLeftTitle: '销量(瓶(支))',
                    yRightTitle: '销售额(万元)',
                  ...
                  this.ypfx.chart.qfsj_xl = Object.assign(this.ypfx.chart.qfsj_xl, {
                    legend: t.x,
                    series: [
                      {
                        name: '销量',
                  ...
                  this.ypfx.chart.qfsj_xse = Object.assign(this.ypfx.chart.qfsj_xse, {
                    legend: t.x,
                    series: [
                      {
                        name: '销售额',
              ...
              case '企业分析':
                if (this.qyfx.val2 == 'all') {
                  ...
                  this.qyfx.chart.qfsj_jd = Object.assign(this.qyfx.chart.qfsj_jd, {
                    legend: ['销量(瓶(支))', '销售额(万元)'],
                    x: t.x,
                    yLeftTitle: '销量(瓶(支))',
                    yRightTitle: '销售额(万元)',
                  ...
                  this.qyfx.chart.qfsj_xl = Object.assign(this.qyfx.chart.qfsj_xl, {
                    legend: t.x,
                    series: [
                      {
                        name: '销量',
                  ...
                  this.qyfx.chart.qfsj_xse = Object.assign(this.qyfx.chart.qfsj_xse, {
                    legend: t.x,
                    series: [
                      {
                        name: '销售额',
      ...
      drawYpfxXszl () {
        ...
        let echart = Echarts.init(this.$refs.ypfxChart1);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计',
      ...
      drawYpfxGgxs () {
        ...
        let echart = Echarts.init(this.$refs.ypfxChart2);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计',
      ...
      drawYpfxGgxsXl () {
        if (!this.$refs.ypfxChart3) {
          ...
          $(this.$refs.ypfxChart3).removeAttr('_echarts_instance_');
        let echart = Echarts.init(this.$refs.ypfxChart3);
        echart.clear();
        echart.setOption({
            title : {
                text: '销量占比',
      ...
      drawYpfxGgxsXse () {
        if (!this.$refs.ypfxChart4) {
          return;
            ...
            $(this.$refs.ypfxChart4).removeAttr('_echarts_instance_')
        let echart = Echarts.init(this.$refs.ypfxChart4);
        echart.clear();
        echart.setOption({
            title : {
                text: '销售额占比',
      ...
      drawYpfxQfsj () {
        ...
        let echart = Echarts.init(this.$refs.ypfxChart5);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计(年统计)',
      ...
      drawYpfxQfsjJd () {
        ...
        let echart = Echarts.init(this.$refs.ypfxChart6);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计(季度统计)',
      ...
      drawYpfxQfsjXl () {
        ...
        $(this.$refs.ypfxChart7).removeAttr('_echarts_instance_');
        let echart = Echarts.init(this.$refs.ypfxChart7);
        echart.clear();
        echart.setOption({
            title : {
                text: '销量占比',
      ...
      drawYpfxQfsjXse () {
        if (!this.$refs.ypfxChart8) {
          return;
            ...
            $(this.$refs.ypfxChart8).removeAttr('_echarts_instance_')
        let echart = Echarts.init(this.$refs.ypfxChart8);
        echart.clear();
        echart.setOption({
            title : {
                text: '销售额占比',
      ...
      drawYpfxJck () {
        ...
        let echart = Echarts.init(this.$refs.ypfxChart9);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计',
      ...
      drawQyfxXszl () {
        ...
        let echart = Echarts.init(this.$refs.qyfxChart1);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计',
      ...
      drawQyfxXszlXl () {
        ...
        $(this.$refs.qyfxChart2).removeAttr('_echarts_instance_')
        let echart = Echarts.init(this.$refs.qyfxChart2);
        echart.clear();
        echart.setOption({
            title : {
                text: '销量占比',
      ...
      drawQyfxXszlXse () {
        ...
        $(this.$refs.qyfxChart3).removeAttr('_echarts_instance_')
        let echart = Echarts.init(this.$refs.qyfxChart3);
        echart.clear();
        echart.setOption({
            title : {
                text: '销售额占比',
      ...
      drawQyfxQfsj () {
        ...
        let echart = Echarts.init(this.$refs.qyfxChart4);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计(年统计)',
      ...
      drawQyfxQfsjJd () {
        ...
        let echart = Echarts.init(this.$refs.qyfxChart5);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计(季度统计)',
      ...
      drawQyfxQfsjXl () {
        if (!this.$refs.qyfxChart6) {
          return;
            ...
            $(this.$refs.qyfxChart6).removeAttr('_echarts_instance_')
        let echart = Echarts.init(this.$refs.qyfxChart6);
        echart.clear();
        echart.setOption({
            title : {
                text: '销量占比',
      ...
      drawQyfxQfsjXse () {
        if (!this.$refs.qyfxChart7) {
          return;
            ...
            $(this.$refs.qyfxChart7).removeAttr('_echarts_instance_')
        let echart = Echarts.init(this.$refs.qyfxChart7);
        echart.clear();
        echart.setOption({
            title : {
                text: '销售额占比',
      ...
      drawQyfxJplb () {
        ...
        let echart = Echarts.init(this.$refs.qyfxChart8);
        echart.clear();
        echart.setOption({
          title: {
                text: '销售统计',
