  ...
  <div class="la-list">
    <List :loading="listLoading" :nodata="res1 && !res1.length">
      <div class="actions" :style="posi">
        <!-- 显示 -->
        <list-check :defaultFiled="defaultFiled" vuex_name="Targetdatas" :showArray="tableHead" @changeTableHeader="(list)=>changeTableHeader(this,list)" style="text-align:left;" />
        <!-- 导出 -->
        <Export
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <div v-if="item.prop === 'target'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)" class="cell-shoulihao-wrap">
                <router-link tag="a" :to="{path:'/targetdatas/' + scope.row.id}" class="no-underline cl-blue" target="_blank">
                  {{ scope.row.target }}
                </router-link>
              </span>
            ...
            <div v-if="item.prop === 'genename'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)">{{ scope.row.genename }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-if="item.prop === 'kind'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)">{{ scope.row.kind }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
             <div v-if="item.prop === 'higheststatus'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)">{{ scope.row.higheststatus }}</span>
              <span v-else class="abandon-click-method">暂无权限</span>
            </div>
            <div v-if="item.prop === 'count_xgyw'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)">
                <router-link v-if="Number(scope.row.count_xgyw)" tag="a" :to="{path:'/report?target_eq=' + scope.row.target}" target="_blank" class="under_a">
            ...
            <div v-if="item.prop === 'count_sbzs'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)">
                <router-link v-if="Number(scope.row.count_sbzs)" tag="a" :to="{path:'/zhuce?targets_eq=' + scope.row.target}" target="_blank" class="under_a">
            ...
            <div v-if="item.prop === 'count_lcsy'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)">
                <router-link v-if="Number(scope.row.count_lcsy)" tag="a" :to="{path:'/linchuangshiyan?targets_eq=' + scope.row.target}" target="_blank" class="under_a">
            ...
            <div v-if="item.prop === 'count_zgss'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)">
                <router-link v-if="Number(scope.row.count_zgss)" tag="a" :to="{path:'/cfdadrug?targets_eq=' + scope.row.target}" target="_blank" class="under_a">
            ...
            <div v-if="item.prop === 'count_qqlc'">
              <span v-if="vueCheckListPms('Targetdatas', item.label)">
                <router-link v-if="Number(scope.row.count_qqlc)" tag="a" :to="{path:'/clinical?targets_eq=' + scope.row.target}" target="_blank" class="under_a">
  ...
  export default{
    ...
    computed: {
      ...
      ...mapState({
        ...
        res1: state => _.keys(state.Targetdatas.res1).length ? state.Targetdatas.res1.res : [],
