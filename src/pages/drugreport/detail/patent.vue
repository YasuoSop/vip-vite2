  ...
  <div class="detail-items" id="detail-items">
    ...
    <div class="detail-item mb50" v-show="cnListRes && cnListRes.totle">
      <div class="item-title">
        <span class="text"
          >中国专利<span class="num">{{ cnListRes.totle }}</span></span
        >
        <span
          class="export"
          v-if="!isPdf"
          @click="maxTotal(exportUrl.cn, 'cn', cnListRes.totle)"
      ...
      <div class="table-wrapper">
        ...
        <pdfTable
          v-else-if="isPdf"
          :headerList="tableHead.cn"
          :dataList="cnListRes.lists"
        />
        <!-- 加载完成显示部分 -->
        <el-table
          :data="cnListRes.lists"
          stripe
          fixed
          class="table"
          style="width: 100%"
    ...
    <div class="detail-item mb50" v-show="usListRes && usListRes.count">
      <div class="item-title">
        <span class="text"
          >美国专利<span class="num">{{ usListRes.count }}</span></span
        >
        <span
          class="export"
          v-if="!isPdf"
          @click="maxTotal(exportUrl.us, us, usListRes.count)"
          ><i class="iconfont">&#xe605;</i> 导出</span
        >
      </div>
      <div class="table-wrapper">
        ...
        <pdfTable
          v-else-if="isPdf"
          :headerList="tableHead.us"
          :dataList="usListRes.res"
        />
        <!-- 加载完成显示部分 -->
        <el-table
          :data="usListRes.res"
    ...
    <div class="detail-item" v-show="allListRes && allListRes.totle">
      <div class="item-title">
        <span class="text"
          >世界专利<span class="num">{{ allListRes.totle }}</span></span
        >
        <span
          class="export"
          v-if="!isPdf"
          @click="maxTotal(exportUrl.all, 'all', allListRes.totle)"
          ><i class="iconfont">&#xe605;</i> 导出</span
        >
      </div>
      ...
      <div class="table-wrapper">
        ...
        <pdfTable
          v-else-if="isPdf"
          :headerList="tableHead.all"
          :dataList="allListRes.lists"
        />
        <!-- 加载完成显示部分 -->
        <el-table
          :data="allListRes.lists"
          stripe
          fixed
          class="table"
          style="width: 100%"
        ...
        <el-pagination
          v-if="allListRes.totle > 10 && !isPdf"
          layout="total, prev, pager, next"
          @current-change="allCurrentChange"
          :current-page="allParam.page"
          :page-size="allParam.pageSize"
...
export default {
  ...
  data() {
    return {
      ...
      exportUrl: {
        cn: this.PATAPI_PROXY + "/dbexport?sourceType=cn",
        us: "/api/report/patentus?outdata=1",
        all: this.PATAPI_PROXY + "/dbexport?sourceType=all",
      },
      cn: {
        page: 1,
        res: {},
