<template>
  <div class="mc">
    <div class="left-nav">
      ...
      <router-link to="/usercenter/take/new" class="nav-lan nav-lan-active">新增订阅&nbsp;&nbsp;></router-link>
    </div>
    <!--  -->
    <div class="right-view">
      ...
      <div class="section">
        <div class="head">
          <div class="title"><i class="iconfont">&#xe757;</i><span>新增订阅</span></div>
        </div>
        <div class="sec-view">
          <div class="form">
            <el-form ref="newTakeForm" :model="newTakeForm" label-width="120px">
              <el-form-item label="订阅数据库：">
                <el-select v-model="shujuku" placeholder="请选择数据库" :clearable="true" @change="rssRules">
              ...
              <el-form-item label="接收频率：">
                <el-radio-group v-model="rate">
                  <el-radio :label="1" value="1">有状态变化时</el-radio>
                </el-radio-group>
              </el-form-item>
              <el-form-item label="接收方式：">
                <el-checkbox-group v-model="mode">
                  ...
                  <el-checkbox :label="3">
                    <span>邮件接收&nbsp;</span>
                    <el-input v-model="email" placeholder="请输入您的邮箱号"></el-input>
                  </el-checkbox>
                </el-checkbox-group>
              </el-form-item>
              <el-form-item>
                <el-button type="primary" @click="submitForm('newTakeForm')" class="tj-btn">提交</el-button>
              </el-form-item>
            </el-form>
          </div>
        </div>
...
export default {
  data () {
    return {
      ...
      qymckeyword: '',
      rate: 1,
      mode: [2], // TODO:暂时隐藏邮件接收，所以暂时从数组中删掉3
      email: '',
      param: {
        page: 1,
        pageSize: 10,
  ...
  methods: {
    ...
    querySearchAsync(queryString, callback) {
      ...
      switch (this.type)
      {
        case 1: type_str = "shoulihao"; break;
        case 2: type_str = "name"; break;
        case 3: type_str = "zwylbm"; break;
        case 4: type_str = "qiyemingcheng"; break;
      }
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (_.indexOf(this.mode, 3) !== -1) { // 接收方式为邮箱时
            _.assign(otherParams, {send_email: this.email})
          }

          switch (this.type)
          {
            case 1: _.assign(otherParams, {shoulihao: this.slhkeyword}); break;
            case 2: _.assign(otherParams, {name: this.namekeyword}); break;
            case 3: _.assign(otherParams, {zwylbm: this.hxcfkeyword}); break;
            case 4: _.assign(otherParams, {qiyemingcheng: this.qymckeyword}); break;
            default: _.assign(otherParams, {shoulihao: this.slhkeyword}); break;
          }

          let testEmailReg = new RegExp(/^[0-9A-Za-z][\.-_-0-9A-Za-z]*@[0-9A-Za-z]+(\.[0-9A-Za-z]+)+$/) // 验证邮箱格式
          let isEmailRight = testEmailReg.test(this.email) // 邮箱格式是否正确

          // if (this.email && isEmailRight && this.mode.length) {

          // }
          if (_.indexOf(this.mode, 3) !== -1 && this.email && !isEmailRight) { // 请输入正确的邮箱
            this.toggleAlert('wrong6')
          } else if (_.indexOf(this.mode, 3) !== -1 && !this.email) { // 请输入邮箱
            this.toggleAlert('wrong5')
          } else if (!this.mode.length) { // 请选择接收方式
            ...
            window.Axios({
              ...
              } else if (res.data.code === 10002) { // 关键词不能为空
                this.toggleAlert('wrong1')
              } else if (res.data.code === 10003) { // 您已经订阅过该关键词
                this.toggleAlert('wrong2')
              } else if (res.data.code === 10004) { // 请输入正确关键词
