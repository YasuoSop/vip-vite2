      ...
      <div class="section">
        ...
        <div class="sec-view" v-else>
          <div class="lan" :class="{'readed': item.status === 1}" v-for="(item, index) in data.res" :key="index">
            ...
            <div class="content">
              <!-- 按受理号订阅 && 受理号变化 -->
              <router-link v-if="item.field == 'shoulihao' && item.type == 1" :to="{path: '/usercenter/take/dynamic/slh/' + item.id}" class="main">
                {{item.title}}
                <span class="topic"><span v-if="item.type_title">#</span>{{item.type_title}}</span>
              </router-link>
              <!-- 按受理号订阅 && 新申请 -->
              <router-link v-if="item.field == 'shoulihao' && item.type == 2" :to="{path: '/zhuce/' + item.infoid, query:{msg_readid: item.id}}" class="main" @click="changeToReaded(item.type_title, item.id)" target="_blank">
                {{item.title}}
                <span class="topic"><span v-if="item.type_title">#</span>{{item.type_title}}</span>
              </router-link>
              <!-- 非按受理号订阅 -->
