  ...
  <div class="la-mc">
    <div class="header">
      <div class="header-main">
        <div class="photo">
          <img :src="vueGetCookie('userPicture')">
        </div>
        <div class="name">{{vueGetCookie('userName')}}</div>
        <div class="power">
          <img :src="'/static/imgs/usercenter/'+vip_img.name" :style="vip_img.style" :title="accountData.grade_name">
          <span>会员等级：{{accountData.grade_name}}</span>
          <span @click="onRenew" v-if="lessThanTheTime(accountData.endtime)">点击续约</span>
        </div>
        <div class="acorss-time" v-if="accountData.endtime != '' && accountData.starttime != ''">
          <span>{{accountData.starttime ? accountData.starttime : "-"}}</span>
          &nbsp;至&nbsp;
          <span>{{accountData.endtime ? accountData.endtime : "-"}}</span>
          <el-tooltip class="item" effect="light" content="尊敬的vip会员，您的产品使用期限即将到期，请及时联系客服续约，以免影响您的正常使用。" placement="right" v-if="lessThanTheTime(accountData.endtime)">
      ...
      <div class="header-nav">
        <router-link to="/usercenter/message" class="nav-item">
          <i class="iconfont">&#xe6eb;</i>
          <span class="nav-name">消息通知<span class="nav-num" v-if="xxtzNumUnread">{{xxtzNumUnread}}</span></span>
        </router-link>
        <!-- 暂不上线，暂时隐藏 -->
        <router-link to="/usercenter/take" class="nav-item">
          <i class="iconfont">&#xe66e;</i>
          <span class="nav-name">订阅管理<span class="nav-num" v-if="dyglNum">{{dyglNum}}</span></span>
        </router-link>
        <!-- 暂未开发，暂时隐藏 -->
        ...
        <router-link to="/home" class="nav-item" @click.native="noviceEvent">
          <i class="iconfont" style="font-size: 14px;height: 25px;">&#xe693;</i>
...
import RenewBase from './renew/base.vue'
