          ...
          <el-table
            ...
            v-for="item in tableHead"
              ...
              <template slot-scope="scope">
                ...
                <div v-if="item.prop === 'zhuangtai'" :style="{color: (scope.row.isreply == 1 ? '#90c320' : '#f77d54')}">
                  {{ scope.row.isreply == 1 ? '已回复' : '未回复' }}
                </div>
                <div v-if="item.prop === 'caozuo'">
                  <el-button type="primary" round size="mini" class="usercenter-suggest-view-btn" @click.prevent="handleSuggestView(scope.row.id)">
                    查看
                  </el-button>
                </div>
              </template>
