        ...
        <div class="sec-view" v-if="xxtzNum">
          <div class="lan" :class="{'readed': item.status === 1}" v-for="(item, index) in data.res" :key="index">
            ...
            <div class="content">
              ...
              <!-- 系统消息 -->
              <router-link v-if="item.dbid === 0 && item.type !== 13" :to="{path:'/usercenter/message/'+item.id,query:{
              ...
              <!-- 反馈回复 -->
              <router-link v-else-if="item.dbid === 0 &&item.type === 13" :to="'/usercenter/suggest/'+item.infoid+'-'+item.id" class="main" :data-url="'/api/usercenter/jyfkdetail'" :data-id="item.id" >
                {{item.title}}
                <span class="topic"><span v-if="item.type_title">#</span>{{item.type_title}}</span>
              </router-link>
              <!-- 注册数据库 - 按受理号订阅 && 受理号变化 -->
              <router-link v-else-if="item.dbid !== 0 && item.field == 'shoulihao' && item.type == 1" :to="{path: '/usercenter/take/dynamic/slh/' + item.id}" class="main" :data-url="'/api/rss/zhuceshouli'" :data-id="item.id" >
                {{item.title}}
                <span class="topic"><span v-if="item.type_title">#</span>{{item.type_title}}</span>
              </router-link>
              <!-- 注册数据库 - 按受理号订阅 && 新申请 -->
              <router-link v-else-if="item.dbid !== 0 && item.field == 'shoulihao' && item.type == 2" :to="{path: '/zhuce/' + item.infoid}" class="main" target="_blank" :data-url="'/api/rss/zhuce'" :data-id="item.id" >
                {{item.title}}
                <span class="topic"><span v-if="item.type_title">#</span>{{item.type_title}}</span>
              </router-link>
              <!-- 注册数据库 - 非按受理号订阅 -->
              <router-link v-else-if="item.dbid !== 0 && item.field != 'shoulihao'" :to="{path: '/usercenter/take/dynamic/other/' + item.id, query:{title: item.title}}" class="main" :data-url="'/api/rss/zhuce'" :data-id="item.id" >
                {{item.title}}
                <span class="topic"><span v-if="item.type_title">#</span>{{item.type_title}}</span>
              </router-link>
              <!-- 点击标记为已读 -->
