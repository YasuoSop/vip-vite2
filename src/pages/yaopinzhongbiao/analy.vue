  ...
  <div class="analy-wrapeer">
    ...
    <div class="chart">
          ...
          <div class="newtotals-scroll">
            <div class="newtotals-scroll-inner">
              <ul class="total-item">
                ...
                <li class="li2">总计<span>{{ typeof(kshTotal.count) == 'undefined' ? 0 : kshTotal.count }}</span>个</li>
              </ul>
              <ul class="total-item">
                <li class="li1">中标品种</li>
                <li class="li2">总计<span>{{ typeof(kshTotal.count_name) == 'undefined' ? 0 : kshTotal.count_name }}</span>个</li>
              </ul>
              <ul class="total-item">
                <li class="li1">剂型</li>
                <li class="li2">总计<span>{{ typeof(kshTotal.count_jixing) == 'undefined' ? 0 : kshTotal.count_jixing }}</span>个</li>
              </ul>
              <ul class="total-item">
                <li class="li1">药品品规</li>
                <li class="li2">总计<span>{{ typeof(kshTotal.count_yppg) == 'undefined' ? 0 : kshTotal.count_yppg }}</span>个</li>
              </ul>
              <ul class="total-item">
                <li class="li1">中标企业</li>
                <li class="li2">总计<span>{{ typeof(kshTotal.count_qiye) == 'undefined' ? 0 : kshTotal.count_qiye }}</span>个</li>
              </ul>
              <ul class="total-item">
                <li class="li1">发布年份</li>
                <li class="li2">总计<span>{{ typeof(kshTotal.count_year) == 'undefined' ? 0 : kshTotal.count_year }}</span>个</li>
              </ul>
              <ul class="total-item">
                <li class="li1">中标地区</li>
                <li class="li2">总计<span>{{ typeof(kshTotal.count_first) == 'undefined' ? 0 : kshTotal.count_first }}</span>个</li>
              </ul>
              <ul class="total-item">
                <li class="li1">中标价格</li>
                <li class="li2">最新<span>{{ typeof(kshTotal.count_feiyong) == 'undefined' ? 0 : kshTotal.count_feiyong }}</span>元</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <div class="chart_bottom">
        <div class="go-list">
          <router-link tag="a" :to="{path:'/yaopinzhongbiao/list.vue'}">返回列表</router-link>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      title: ["药品排行","企业排行","ATC分类","剂型占比","热力图","中标地区"], //图形标题
      sbqs_line: {
        echarts: null,
        yAxis_data: [],
        series_data: []
  ...
  methods: {
    ...
    drawSbqsLine() {
      var _that = this;
      _that.sbqs_line.echarts = Echarts.init(document.getElementById("sbqs-line"));
    ...
    drawSbqsLineOpen() {
      var _that = this;
      Echarts.init(document.getElementById("sbqs-line-open")).setOption({
        tooltip: {
          trigger: "item",
          formatter: "{b}<br />数量: {c}"
        },
    ...
    drawSbqsBar() {
      var _that = this;
      _that.sbqs_bar.echarts = Echarts.init(document.getElementById("sbqs-bar"));
    ...
    dealSqlxBar(originalData) {
      ...
      if (_len > 0) {
        ...
        for (var i = 0; i < _len; i++) {
          ...
          series_data.push({ value: originalData[i].doc_count, name: originalData[i].key });
        }
        ...
        let xAxis_data2 = [];
        let series_data2 = [];
        let originalData2 = originalData.slice().sort((a, b) => {
        ...
        for (var i = 0; i < _len; i++) {
          ...
          series_data2.push({ value: originalData2[i].doc_count, name: originalData2[i].key });
        }

        // 排序
        for (let index in series_data2) {
          for (let i = 0; i < index; i++) {
            if (Number(series_data2[index].value) > Number(series_data2[i].value)) {
    ...
    drawSqlxBar() {
      var _that = this;
      _that.sqlx_bar2.echarts = Echarts.init(document.getElementById("sqlx-bar"));
    ...
    drawYplxBar() {
      var _that = this;
      _that.yplx_bar2.echarts = Echarts.init(document.getElementById("yplx-bar"));
      ...
      if (d.length > 0) {
        ...
        _.forEach(d, (curr, index, arr) => {
          ...
          _.forEach(curr.year.buckets, (cur, inde, ar) => {
            ...
            // 记录格子中的最大值
            this.spjl_pie.max_value = this.spjl_pie.max_value < cur.doc_count ? cur.doc_count:this.spjl_pie.max_value;
          })
        })
        ...
        _.forEach(d2, (curr, index, arr) => {
          ...
          _.forEach(curr.year.buckets, (cur, inde, ar) => {
            ...
            // 记录格子中的最大值
            this.spjl_pie2.max_value = this.spjl_pie2.max_value < cur.doc_count ? cur.doc_count:this.spjl_pie2.max_value;
          })
        })
        // y 轴年份转换为数组
        y2 = _.keys(y2);
    ...
    drawSpjlPie() {
      var _that = this;
      _that.spjl_pie2.echarts = Echarts.init(document.getElementById("spjl-pie"));
        ...
        series: [{
          ...
          tooltip: {
            formatter: function (params) {
              ...
              return '<b>'+params.name+'</b>地区在'+'<b>'+_that.spjl_pie2.y[params.value[1]]+'</b>年<br/>中标品种数量是'+'<b>'+params.value[2]+'</b>（个）'
            }
          },
          label: {
            show: true,
        ...
        series: [{
          ...
          tooltip: {
            formatter: function (params) {
              ...
              return '<b>'+params.name+'</b>地区在'+'<b>'+_that.spjl_pie.y[params.value[1]]+'</b>年<br/>中标品种数量是'+'<b>'+params.value[2]+'</b>（个）'
            }
          },
          label: {
            show: true,
    ...
    dealSbdqMap(originalData) {
      ...
      if (_len > 0) {
        ...
        for (var i = 0; i < _len; i++) {
          ...
          max = originalData[i].doc_count > max ? originalData[i].doc_count : max;
        }
    ...
    drawSbdqMap() {
      var _that = this;
      _that.sbdq_map.echarts = Echarts.init(document.getElementById("sbdq-map"));
  ...
  watch: {
    ...
    $route(){
      if(this.$route.path==='/yaopinzhongbiao/analy'&&Object.keys(this.kshRes).length<=1){
        this.loadData()
      }
      if (this.$route.path === '/yaopinzhongbiao/analy') {
        window.addEventListener('resize', this.echartsResizeHandler);
