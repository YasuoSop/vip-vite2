        ...
        <el-table-column
         v-for="item in tableHead"
          ...
          <template slot-scope="scope">
            ...
            <div v-if="item.prop === 'qyfx'" @click="showAnaly(scope.row.qiye)">
              <router-link target="_blank" class="cl-blue" :to="{ path: '/yyxsdata', query: { tab: '2',qyname:scope.row.guifanqiye }}">企业分析</router-link>
            </div>
            <div v-else :class="{'extensible-td': Number(scope.row[item.prop])}" @click="item.prop != 'guifanqiye'?getExpandRes(scope.row.guifanqiye, item.prop, scope.row[item.prop], item.label,scope):''">
              {{ scope.row[item.prop] }}
            </div>
            </div>
          </template>
  ...
  export default{
    ...
    computed: {
      ...
      ...mapState({
        ...
        res3: state => _.keys(state.Yyxs.res3).length ? state.Yyxs.res3.res : [],
    ...
    methods: {
      ...
      handleRequest (props) {
        Store.dispatch("Yyxs/getGroupRes",{path:'/api/yyxs/byqiye',params:this.expandParams}).then(res=>{
      ...
      sortChange (column, prop, order) {
        ...
        Store.commit('Yyxs/order', Object.assign({}, this.$store.state.Yyxs.order, {qiye: sort ? sort : null}))
        Store.dispatch('Yyxs/nomalSearch',{params:this.param})
      },
      getExpandRes (drugsname,groupname,num,label,scope) {
        if(num){
          ...
          this.expandFiled[0] = {prop:'field', label:label, width:100, checked:true}
      ...
      toggleRowExpansion(row, column, cell, event){
        ...
        window.ga("send","event", "txt", "click","yyxs_"+"_byqiye_"+column.property);
      window._paq.push(['trackEvent', 'txt',"click","yyxs_"+"_byqiye_"+column.property, ])
