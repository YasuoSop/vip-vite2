      ...
      <div class="chart_top">
        <div class="totals">
          <ul class="total-item" v-for="(item, i) in totalList" :key="i">
            ...
            <li class="li2">
              总计
              <span>{{ kshResTotal[item.key] }}</span
              >个
            </li>
          </ul>
        </div>
      ...
      <div class="dialog" v-show="dialogVisible">
        <div class="menu">
          <div
            ...
            @click="handleOpenIndex(index, [], true)"
            :key="index"
          >
            {{ ti }}
          </div>
        ...
        <div class="echarts-items-wap">
          ...
          <div class="echarts-items">
            <div
              class="echarts-item"
              v-for="(item, i) in echartsListOpen"
              :key="i"
              v-show="i === openIndex"
            >
              <MEcharts
          ...
          <div class="ctr-actions">
            <el-button
              class="pre action"
              @click="handleOpenIndex(openIndex - 1, [], true)"
            >
...
export default {
  ...
  data() {
    return {
      ...
      kshResTotal: {},
      totalList: [
        { name: "品种数", key: "guifanname" },
        { name: "生产企业", key: "guifanqiye" }
      ],
      echartsList: [
        {
          title: "药品排行",
  ...
  methods: {
    ...
    loadData: _.debounce(function() {
      ...
      // 加载 aggs API的数量总计
      this.promise = Store.dispatch("Yyxs/getKshRes").then(res => {
        this.update(res);
        return res;
      });
      setTimeout(() => {
