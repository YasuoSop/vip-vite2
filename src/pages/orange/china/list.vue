            ...
            <template slot-scope="scope">
              ...
              <div v-if="item.prop === 'name'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">
                  <router-link tag="a" :to="{path:'/orangebook/' + scope.row.id}" target="_blank" class="cl-blue" v-html="scope.row.name"></router-link>
                </span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'jixing'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">{{ scope.row.jixing }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'guige'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">{{ scope.row.guige }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'piwen'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">
                  <a
                  :class="scope.row.piwen == 0?'no_link-underline':'link-underline link-underline_hover'"  @click.prevent="pushRoute('cfdadrug', 'database_cfdadrug', 'CfdaDrug', scope.row.bianma)">{{ scope.row.piwen }}</a>
                </span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'cname'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">{{ scope.row.cname }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'ename'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">{{ scope.row.ename }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'rldf'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">{{ scope.row.rldf }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'rldj'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">{{ scope.row.rldj }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'originalc'">
                <span v-if="vueCheckListPms('Orangebook', item.label)">{{ scope.row.originalc }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'bigdata_analy'">
                <a :href="'https://db.yaozh.com/bigdata/yzxpj?source=db&dbname=orangebook&id=' + scope.row.id" class="cl-blue" target="_blank">查看</a>
              </div>
              </div>
            </template>
          </el-table-column>
...
<script>
  ...
  const defaultFiled = tablehead.orange.china
  export default {
    components:{
      List,
      Export,
    ...
    computed: {
      ...mapState({
        ...
        res1: state => _.keys(state.Orangebook.res1).length ? state.Orangebook.res1 : [],
