<template>
  <div>
    ...
    >
          ...
          <el-collapse-item title="剂型" name="type">
            <div class="keyword">
              <div v-for="(item, idx) in types" :key="item.value">
                <el-checkbox v-show="idx < hideTypes" v-model="item.checked"
                  >{{ item.label }}({{ item.nums }})</el-checkbox
                >
              </div>
              <div class="controls">
                <a v-if="types.length > 5" @click="hideChange('types')"
                  ><span>{{ hideTypes == 5 ? "更多" : "收起" }}</span
                  ><i class="icon el-icon-d-arrow-right"></i
                ></a>
                <el-button
                  size="mini"
                  type="primary"
                  @click="searchForChartTypes"
                >
                  筛选
                </el-button>
              </div>
          ...
          <el-collapse-item name="area">
            ...
            <div class="keyword">
              <div v-for="(item, idx) in areas" :key="item.value">
                <VCheckBox
                  v-show="idx < hideAreas"
                  :value="item.checked"
                  @change="checkedChange($event, idx, item)"
      ...
      <template v-else>
        <div
          style="padding: 15px; text-align: center; font-size: 12px;color: #999"
        >
          暂无数据...
        </div>
      </template>
...
export default {
  ...
  methods: {
    ...
    searchForChartTypes() {
      ...
      parent.inputChange(parent.keyword, parent.types, parent.areas);
      this.$emit("search", this.types, this.areas);
    },
    searchForChartAreas() {
      let parent = this.$parent.$parent.$parent;
      parent.inputChange(parent.keyword, parent.types, parent.areas);
