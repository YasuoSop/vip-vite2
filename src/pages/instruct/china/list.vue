    ...
    <List :loading="listLoading" :nodata="res1 && !res1.length">
      <div class="item_content1">
        ...
        <div>
                  ...
                  <div v-if="item.prop=='name'">
                    <div class="table-name">
                      <a
                        ...
                        class="cl-blue"
                        @click="handleCardClick(scope.row.id,scope.row.xiangqing)"
                      >{{ scope.row.name }}{{scope.row.shangpinname ? `(${scope.row.shangpinname})` : ''}}</a>
                      <a
                        v-if="scope.row.xiangqing===0"
                        class="cl-blue"
                        href="javascript:void;" @click="downloadFile(scope.row.down)"
                      >{{ scope.row.name }}{{scope.row.shangpinname ? `(${scope.row.shangpinname})` : ''}}</a>
                      <span
                      ...
                      <span
                        ...
                        title="文本"
                      >文</span>
                      <span class="instruct-icon" v-if="scope.row.yibao" title="国家医保">医保</span>
                    ...
                    <!-- <div>
                      ...
                      <span
                        ...
                        title="文本"
                      >文本</span>
                      <span class="instruct-icon" v-if="scope.row.yibao" title="国家医保">国家医保</span>
                      <span
                        class="instruct-icon icon-orange"
                        v-if="scope.row.otcsort"
                        :title="scope.row.otcsort"
                      >{{scope.row.otcsort}}</span>
                    </div> -->
                  </div>
                  <div v-else-if="item.prop=='zuixinxiugairi'" :title="getAllXiugairi(scope.row.xiugairi)"
                  >{{scope.row.zuixinxiugairi?scope.row.zuixinxiugairi:'/'}}</div>

                  <!-- 放开后显示包装图片 -->
                  <div v-else-if="item.prop=='urls'">
                    <imgShow :url="scope.row.urls ? scope.row.urls : scope.row.url"></imgShow>
                  </div>

                  <div v-else>
                    <span v-if="vueCheckListPms('ShuomingshuZg', item.label)">{{ scope.row[item.prop] }}</span>
          ...
          <div class="page-cards" :style="{height:tableHeight+'px'}" v-show="!isTableCard">
            <!-- xiangqing为1时才能点击进入详情页 -->
            <div
              class="card"
              :class="{'card-no-detail': item.xiangqing===0}"
              v-for="(item, index) in res1"
              :key="index"
            >
              <a @click="handleCardClick(item.id,item.xiangqing)" class="card-top">
                <div class="guding-tip">CHINA</div>
                <div class="download" v-if="item.down">
                  <a :href="item.down" class="fujian" target="_blank">
                    <i class="iconfont download-icon">&#xe62c;</i>
                  </a>
                  <!-- <div class="fujian-cl"></div> -->
                </div>
                <div class="drug-name" :title="'药品名称：' + item.name">{{item.name}}</div>
                <div
                  :class="{'drug-noyibao':!item.yibao&&!item.otcsort}"
                  class="drug-source"
                  :title="'说明来源：' + item.source"
                >{{item.source}}</div>
                <div class="drug-yibao">
                  <span class="instruct-icon" v-if="item.yibao" title="国家医保">国家医保</span>
                  <span
                    class="instruct-icon icon-orange"
                    v-if="item.otcsort"
                    :title="item.otcsort"
              ...
              <div class="card-bottom">
                <a class="bottom-item" @click.prevent="clickToSearch(item.ylbm, item.china)">
...
import imgShow from "@/components/common/imgShow.vue"
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      if (val * this.param.pageSize <= 1500) {
        $(".out-of-page").hide();
        Store.dispatch("ShuomingshuZg/nomalSearch", { params: Object.assign({}, this.param, this.sort ? { order: this.sort } : {}) });
      } else {
        $(".out-of-page").show();
      }
    },
    ...
    shoumingshuStatistics() {
      ...
      window._paq.push(['trackEvent', 'button', 'cilick', 'vip_instruct_other', ]);
    }
  },
  created() {
  },
