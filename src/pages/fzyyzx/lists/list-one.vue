<template>
  <div class="la-list">
    <List :loading="listLoading" :nodata="res1 && !res1.length">
      <div class="actions">
        <!-- 显示 -->
        <list-check :defaultFiled="defaultFiled.vue"x_name="Yzx" :showArray="tableHead" @changeTableHeader="(list)=>changeTableHeader(this,list)" />
        <!-- 导出 -->
        <Export
          ...
          <template :slot="item.tips ? 'header':''" slot-scope="scope">
            <span>
              ...
              <el-tooltip class="" effect="zhuce" placement="right">
                <div slot="content" v-if="item.prop == 'dailiangcaigou'">{{helpDesc.dailiangcaigou}}</div>
                <div slot="content" v-else-if="item.prop == 'shuoming'">{{helpDesc.shuoming}}</div>
...
<script>
  ...
  const defaultFiled = tablehead.yzx.pw
  export default{
    components: {
      List,
      Export,
