          ...
          <div class="item" v-for="(key,index) in keys" :key="index">
            ...
            <div v-for="(item,index1) in lists[key]" :key="index1">
              <div class="list-title">
                ...
                <span :class="index==0&&index1==0?'el-icon-caret-bottom':'el-icon-caret-top'" @click="handleSwitch($event)"></span>
              </div>
...
export default{
  ...
  methods: {
    ...
    handleScroll () {
      ...
      if(scrollTop < -252) {
        $('.updatelog_nav').css({"top": 0, "left": 0, "position": "fixed","background":"#4877e8"})
      } else {
        $('.updatelog_nav').css({"top": 0, "left": 0, "position": "absolute","background": "none"})// top最小为49
      }
    },
    // 顶部导航条UI样式自适应处理方法
    handleNavPoint () {
