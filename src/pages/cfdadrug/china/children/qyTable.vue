                ...
                <el-table-column
                  ...
                  <template slot-scope="scope">
                    <div :title="scope.row[item.prop]">
                      <a v-if="item.prop==='pizhunwenhao'" target="_blank" style="color:#4877e8;"
                        :href="`/cfdadrug/${scope.row.table_type===2?'jkdetail':'detail'}/${scope.row.xuhao}?pizhunwenhao=${scope.row.pizhunwenhao}`">
                        {{ scope.row[item.prop] }}
                      </a>
                      <span v-else>
                        {{scope.row[item.prop]}}
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div :title="item.prop==='zdsbpz'||item.prop==='zxzt'?'':scope.row[item.prop]">
              ...
              <div v-else :class="Number((item.prop.indexOf('shijian')!==-1&&Number(scope.row[`${item.prop}_count`]))||(item.prop.indexOf('shijian')===-1&&scope.row[item.prop]))?'extensible-td':''" @click="getExpandRes(scope.row.guifanqiye, item.prop, scope.row,scope)">
  ...
  export default {
    ...
    methods: {
      ...
      sortChange (column) {
        ...
        Store.commit('CfdaDrug/order', Object.assign({}, this.$store.state.CfdaDrug.order, {qy: sort ? sort : null}))
      ...
      objectSpanMethod({ row, column, rowIndex, columnIndex }) {
        ...
        if (columnIndex === 0) {
          if(rowIndex===0){
            if(arr.length===1||arr[rowIndex].name!==arr[rowIndex+1].name){
              return [1,1]
            }else{
              let rowspan = 1
      ...
      getExpandRes (guifanqiye,groupname,row,scope) {
        ...
        let num = groupname.indexOf('shijian')!==-1
        ?Number(row[`${groupname}_count`])
        : Number(row[groupname])
        if (num) {
          this.expands.tableType = groupname.indexOf('pinzhong')!==-1 ? 'yp':'other'
          ...
          this.expandParams.piwenshijian_eq = groupname.indexOf('shijian')!==-1? row[groupname]:''
      ...
      handle_request (scope){
        ...
        Store.dispatch("CfdaDrug/getGroupRes",{path:'/api/cfdadrug/qiyeNameList',params}).then(res=>{
      ...
      toggleRowExpansion(row, column, cell, event){
        ...
        let num = column.property.indexOf('shijian')===-1
        ? Number($(cell).text().trim())
