          ...
          >
            <template slot-scope="scope">
              ...
              <div v-if="item.prop === 'name'">
                <span v-if="vueCheckListPms('Epyp', item.label)">
                  <router-link
                    ...
                    tag="a"
                    :to="{path:'/epyp/emadetail/' + scope.row.id, query: {active: scope.row.active}}"
                    target="_blank"
                    style="color:#4877e8;"
                  ...
                  <router-link
                    ...
                    tag="a"
                    :to="{path:'/epyp/hmadetail/' + scope.row.id, query: {active: scope.row.active}}"
                    target="_blank"
                    style="color:#4877e8;"
                  >{{ scope.row.name }}</router-link>
              ...
              <div v-if="item.prop === 'cname'">
                <span v-if="vueCheckListPms('Epyp', item.label)">{{ scope.row.cname }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'active'">
                <span v-if="vueCheckListPms('Epyp', item.label)">{{ scope.row.active }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'atc'">
                <span v-if="vueCheckListPms('Epyp', item.label)">{{ scope.row.atc }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'date'">
                <span v-if="vueCheckListPms('Epyp', item.label)">{{ scope.row.date }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'changshang'">
                <span v-if="vueCheckListPms('Epyp', item.label)">{{ scope.row.changshang }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'status'">
                <span v-if="vueCheckListPms('Epyp', item.label)">{{ scope.row.status }}</span>
                <span v-else class="abandon-click-method">暂无权限</span>
              </div>
              <div v-if="item.prop === 'targets'" :title="''">
                ...
                <div v-if="item.prop === 'zwform'">
                  <span v-if="vueCheckListPms('Epyp', item.label)">{{ scope.row.zwform||"/" }}</span>
                  <span v-else class="abandon-click-method">暂无权限</span>
                </div>
              </div>
            </template>
...
<script>
  ...
  const defaultFiled = tablehead.cfdadrug.Detailepyp
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        param: state => state.Epyp.param,
        order: state => state.Epyp.order,
        listLoading: state => state.Epyp.listLoading,
        res1: state => (_.keys(state.Epyp.res1).length ? state.Epyp.res1 : [])
      }),
      exportDefaultFiled() {
        let arr = [];
