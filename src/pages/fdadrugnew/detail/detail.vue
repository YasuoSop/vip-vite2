    ...
    <div class="detail-list" v-else>
      <div class="detail-header">
        <div class="header-left">
          <div class="left-top">
            <span class="top-id" :title="base_info.drugname">{{base_info.drugname}}</span>
            <span class="top-num">{{base_info.applno}}</span>
            <div class="left-bottom">
              <span class="id-green bg-blue" v-if="base_info.firstgeneric === '是'">首仿</span>
              <span class="id-green bg-orange" v-if="base_info.chengpishu === '是'">橙皮书</span>
              <span class="id-green" v-if="base_info.submissionpropertytypecode === '是'">孤儿药</span>
              <span class="id-green bg-blue" v-if="base_info.essentialmedicine !== ''">基药</span>
              <span class="id-green bg-orange" v-if="base_info.firstinclass !== ''" title="首创新药">首创</span>
              <span class="id-green" v-if="base_info.firstapproved !== ''" title="美国首批">首批</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="base-info">
          <div class="base-info-ou">
            <ul class="base-info-in1" style="width: 25%;">
              <li>
                <div class="flex">
                  <label>申请号</label>
                  <div class="flex1"><span>{{base_info.applno}}</span></div>
                </div>
              </li>
              <li>
                <div class="flex">
                  <label>申请类型</label>
                  <div class="flex1"><span>{{base_info.appltype}} {{base_info.appltype_cn}}</span></div>
                </div>
              </li>
              <li>
                <div class="flex">
                <label>活性成分</label>
                  <div class="flex1"><span>
              ...
              <li>
                <div class="flex">
                  <label>剂型/给药途径</label>
                  <div class="flex1"><span>{{base_info.route}}</span></div>
                </div>
              </li>
              <li>
                <div class="flex">
                  <label>申请机构</label>
                  <div class="flex1"><span>
                    <router-link v-if="base_info.sponsorname" tag="a" :to="{path: `/fdadrugnew?sponsorname=${base_info.sponsorname}&tabName=jigouGroup`}" target="_blank" class="link-a">
                      {{base_info.sponsorname}}
                    </router-link>
                    {{base_info.chinesecompany !== "否" ? base_info.chinesecompany : ""}}
                  </span></div>
                </div>
              </li>
              <li>
                <div class="flex">
                  <label>批准日期</label>
                  <div class="flex1"><span>{{base_info.submissionstatusdate1}}</span></div>
                </div>
              </li>
            </ul>
            ...
            <ul class="base-info-in1 flex1">
              <li>
                <div class="flex">
                  ...
                  <div class="flex1 t-line5">
                    <p v-for="(item, ix) in base_info.targets" :key="ix">
                    <span v-if="item.target_url"><router-link tag="a" :to="{path: item.target_url}" target="_blank" class="link-a">
                      {{item.target+';'}}
                    </router-link> </span>
              ...
              <li>
                <div class="flex">
                  <label>靶点简称</label>
                  <div class="flex1 t-line5">
                    <p v-for="(item, ix) in base_info.targets_abbr" :key="ix">
                     <span v-if="item.target_url"> <router-link tag="a" :to="{path: item.target_url}" target="_blank" class="link-a">
                        {{item.target+';'}}
                      </router-link> </span>
                      <span v-else>{{item.target+';'}}</span>
          ...
          <ul class="base-info-in2">
            <li v-if="base_info.clinical">
              <label>临床试验</label>
              <div class="p-cont t-line5"><p>{{base_info.clinical}}</p></div>
            </li>
            <li v-if="base_info.summary">
              <label>药物概述</label>
              <div class="p-cont t-line5"><p>{{base_info.summary}}</p></div>
            </li>
          </ul>
        </div>
        <div class="classified-in">
          <div class="left-nav">
            <div class="nav-list nav-event" @click="handleNavClick">
              <a class="nav-item active" data-ix="0">药品申请与批准</a>
              <a class="nav-item" data-ix="1" v-if="ndc.List.length">药品NDC信息</a>
              <a class="nav-item" data-ix="2" v-if="ypxxListData.List.length">药品规格信息</a>
              <a class="nav-item" data-ix="3" v-if="equivalent.List.length">治疗等效性相关药品信息</a>
              <a class="nav-item" data-ix="4" v-if="lsspListData.List.length">原始申请或者临时审批</a>
              <a class="nav-item" data-ix="5" v-if="splsListData.List.length">审批历史</a>
              <a class="nav-item" data-ix="6" v-if="hasExtend">扩展信息</a>
            </div>
          </div>
          <div class="right-list">

            <slide-section :title="'药品申请与批准'" class="the-part">
              <div class="tb-wrap">
                <table class="tb-t">
                  <tr>
                    <td>基药</td>
                  ...
                  <tr>
                    ...
                    <td>{{ base_info.firstinclass }}</td>
                    <td>
                      美国首批
                      <el-tooltip effect="light" placement="right">
                        <div slot="content" v-html="tooltip.firstapproved"></div>
                        <i class="el-icon-question cl-green"></i>
                      </el-tooltip>
                    </td>
                    <td>{{ base_info.firstapproved }}</td>
                  </tr>
                  <tr>
                    <td>孤儿药</td>
                    <td>{{base_info.submissionpropertytypecode}}</td>
                    <td>
                      首仿药
                      <el-tooltip effect="light" placement="right">
                  ...
                  <tr>
                    ...
                    <td>{{base_info.f505b2}}</td>
                  </tr>
                  <tr>
                    <td>
                      突破性疗法
                      <el-tooltip effect="light" placement="right">
                        <div slot="content" v-html="tooltip.breakthroughtherapy"></div>
                        <i class="el-icon-question cl-green"></i>
                      </el-tooltip>
                    </td>
                    <td>{{base_info.breakthroughtherapy}}</td>
                    <td>
                      优先审评
                      <el-tooltip effect="light" placement="right">
                  ...
                  <tr>
                    ...
                    <td>{{base_info.fasttrack}}</td>
                  </tr>
                  <tr>
                    <td>
                      市场状态
                      <el-tooltip effect="light" placement="right">
                        <div slot="content" v-html="tooltip.marketingstatusid"></div>
                        <i class="el-icon-question cl-green"></i>
                      </el-tooltip>
                    </td>
                    <td>{{base_info.marketingstatusid}}</td>
                    <td>
                      非排他性
                      <el-tooltip effect="light" placement="right">
                        <div slot="content" v-html="tooltip.offexclusiity"></div>
                        <i class="el-icon-question cl-green"></i>
                      </el-tooltip>
                    </td>
                    <td>{{base_info.offexclusiity}}</td>
                  </tr>
                  <tr>
                    <td>
                      PDUFA
                    ...
                    <td>{{base_info.firstcycle}}</td>
                  </tr>
                </table>
              </div>
            </slide-section>

            <slide-section :title="'药品NDC信息'" class="the-part" v-if="ndc.List.length">
              <div class="tb-wrap">
                <el-table
                  :data="ndc.List"
                  stripe
                  ...
                  <el-table-column
                    ...
                    >
                      <template slot-scope="scope">
                        <div v-if="item.prop === 'productndc'">
                          <a v-if="scope.row.productndc_url" target="_blank" :href="scope.row.productndc_url" class="link-a1">{{ scope.row.productndc }}</a>
            ...
            <slide-section :title="'药品规格信息'" class="the-part" v-if="ypxxListData.List.length">
                    ...
                    <template :slot="item.tips ?'header':''">
                      ...
                      <template slot-scope="scope">
                        <div v-if="item.prop === 'patent'">
                          <a class="linkPrimaryColor" v-if="scope.row.patent" target="_blank" :href="scope.row.patent">查看</a>
                          <span v-else>无</span>
                        </div>
                      <template v-else>
                        <div v-if="item.prop === 'strength'">
                          <a class="linkPrimaryColor" @click.prevent.stop="anchor(scope.row[item.prop])">{{scope.row[item.prop]}}</a>
                        </div>
                        <div v-else>
            ...
            <slide-section :title="'治疗等效性相关药品信息'" class="the-part" v-if="equivalent.List.length">
              <div class="tb-wrap">
                <div class="label-zl">
                  <span v-if="base_info.activeingredient"> <strong>活性成分：</strong>{{base_info.activeingredient}}</span>
                  <span v-if="base_info.route"> <strong>剂型/给药途径：</strong>{{base_info.route}}</span>
                  <span v-if="base_info.tecode">
                    <strong>等效性代码
                       ...
                       ：</strong>{{base_info.tecode}}</span>
                </div>
                <div v-for="(items, ix) in equivalent.List" :id="items.strength" :key="ix" class="equiv-lz" :class="{ selected: curAnchor === items.strength }">
                  <p><span>规格</span>（<span class="fontw"> {{items.strength}} </span>） </p>
                  <el-table
                    :data="items.child"
                      ...
                      >
                        <template slot-scope="scope">
                          <div v-if="item.prop === 'applno'">
                            <a v-if="scope.row.applno_url&&!scope.row.applno_url.includes(base_info.applno)" target="_blank" class="link-a1" :href="scope.row.applno_url">{{ scope.row.applno }}</a>
            ...
            <slide-section :title="'原始申请或者临时审批'" class="the-part" v-if="lsspListData.List.length">
              <div class="tb-wrap">
                <el-table
                  ...
                  <el-table-column
                    ...
                    >
                      ...
                      <template slot-scope="scope">
                        ...
                        <div v-if="item.prop === 'applicationdocsurl'" style="display: -webkit-box;-webkit-box-orient: horizontal;">
            ...
            <slide-section :title="'审批历史'" class="the-part" v-if="splsListData.List.length">
              <div class="tb-wrap">
                <el-table
                  ...
                  <el-table-column
                    ...
                    :label="item.label">
                      <template slot-scope="scope" v-if="scope.row.submissiontype!==''">
                        ...
                        <div v-if="item.prop === 'applicationdocsurl'" style="display: -webkit-box;-webkit-box-orient: horizontal;">
                          <span v-for="(item, index) in scope.row.applicationdocsurl" :key="index">
                            <br v-if="index" />
                            <a :href="item" style="color:#4877e8;" target="_blank" v-html="handleAAA(item)"></a>
                          </span>
                        </div>
                      </template>
                  </el-table-column>
  ...
  export default{
    ...
    data(){
      return{
        ...
        tooltip: {
          ...
          firstinclass: '用一种新的和独特的机制来治疗疾病的药物。',
          firstapproved: '在其他国家批准之前在美国首次批准的药物。',
          pdufa: 'FDA将就每一项申请设置一个审批截止日期(目标日期)，即PDUFA date，为"是"，则该项申请在规定日期截至之内完成了审批。',
          firstcycle: '新药的首轮评审有效地防止了将有价值的新疗法推向市场的延误。',
        },
    ...
    methods:{
      TableRowStyle({row, rowIndex}) {
        return { background: this.curAnchor === row.strength ? 'rgba(255, 245, 242) !important' : '' };
      },
      TableHeaderStyle(val) {
        return { background: this.curAnchor === val ? 'rgba(255, 245, 242) !important' : '' };
      },
      // 锚链接跳转
      anchor(anchorName) {
