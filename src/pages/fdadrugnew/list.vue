<template>
  <div>
    <el-tabs :value="activeTabName" @tab-click="handleClickTag" class="la-list" :before-leave="beforeLeave">
      <el-tab-pane
      ...
      <el-tab-pane
        :label="getTabLabel('按通用名称浏览',extendCount.NameCount)"
        name="nameGroup"
      ...
      <el-tab-pane label="最新批准" name="zuixinpizhun">
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
export default{
  ...
  methods: {
    ...
    handleClickTag (tab,event) {
      let path = tab.name !== 'basegroup' ? "/api/fdadrugnew/" + tab.name : "/api/fdadrugnew/"
      window.ga("send", "event", "tab", "click", "fdadrugnew_"+tab.name)
      window._paq.push(['trackEvent', 'tab', 'click', "fdadrugnew_"+tab.name,])

      Store.commit("Fdadrugnew/tabname",tab.name)
      Store.commit("Fdadrugnew/path",path)
      if ((tab.name === "basegroup" && !_.keys(this.res1).length) || (tab.name === "nameGroup" && !_.keys(this.res2).length) || (tab.name === "jigouGroup" && !_.keys(this.res3).length)) {
        Store.dispatch('Fdadrugnew/nomalSearch',{path:path, queryToPutong: false})
      }
