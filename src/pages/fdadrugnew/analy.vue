  ...
  <div class="analy-wrapeer">
    ...
    <div class="chart">
      ...
      <div class="chart_top">
        <div class="newtotals">
          <ul class="total-item">
            ...
            <li class="li2">总计<span>{{base_count.shangshi_count ? base_count.shangshi_count : 0}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">上市品种</li>
            <li class="li2">总计<span>{{base_count.shangshi_pinzhong_count ? base_count.shangshi_pinzhong_count : 0}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">剂型</li>
            <li class="li2">总计<span>{{base_count.jixing_count ? base_count.jixing_count : 0}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">申请机构</li>
            <li class="li2">总计<span>{{base_count.jigou_count ? base_count.jigou_count : 0}}</span>个</li>
          </ul>
          <ul class="total-item">
            <li class="li1">上市年份</li>
            <li class="li2">总计<span>{{base_count.shangshi_year_count ? base_count.shangshi_year_count : 0}}</span>个</li>
          </ul>
        </div>
      </div>
      <div class="chart_bottom">
        <div class="go-list">
          <router-link tag="a" :to="{path:'/fdadrugnew/list.vue'}">返回列表</router-link>
        </div>
        <div class="chart-boxes">
          <DbEcharts  v-for="(item, ix) in titles" :key="ix" :title="item.name">
            <div class="echarts">
              <div class="title-tooltip">
                <el-tooltip class="item" effect="light" placement="right" v-if="item.tooltip">
                  <div slot="content" v-html="item.tooltip"></div>
                  <i class="el-icon-question cl-green" style="font-size: 14px;"></i>
                </el-tooltip>
              </div>
              <div :id="'bar' + ix" class="barin"></div>
              <div class="nodata-box" v-if="item.show"></div>
              <div class="nodata-box-atc" v-if="item.show_atc"></div>
              <LoadingGif :className="'chart-loading'" :loadFlag="item.loading" v-if="item.loading" :delayTime="0"></LoadingGif>
            </div>
          </DbEcharts>
        </div>
      </div>
      ...
      <div class="dialog" v-if="dialogVisible">
        <div class="menu">
          <div v-for="(ti,index) in titles" v-bind:class="[index==openIndex ? 'active' : '', 'nav']" @click="handleOpenIndex(index)" :key="index">
            {{ti.name}}
          </div>
        </div>
        <div class="echarts-items-wap">
          <div class="echarts-items-title">
            ...
            <a class="esc-btn" href="javascript:;" @click="dialogVisible=false"><i></i><img src="/static/imgs/echarts/esc-fullscreen.png" alt="退出全屏"></a>
          </div>
          <div class="echarts-items1">
            <div id="bigchart" style="height:100%;width: 100%" class="bigchart"></div>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      base_count: state => state.Fdadrugnew.kshRes.base_count ? state.Fdadrugnew.kshRes.base_count : {},
  ...
  methods: {
    ...
    update() {
      let {pinzhong,jigou,shangshi_qushi,jixing,zhiliao_lingyu,submit_type} = this.kshRes
      if(
        ...
        toolbox: {
          ...
          feature: {
            ...
            myTogglePie: {
              ...
              onclick() {
                if(gaparam){
                  window.ga("send", "event", "option", "click", 'cfdadrug_analy_'+ gaparam +'_qiehuan')
                  window._paq.push(['trackEvent', 'option', 'click', 'cfdadrug_analy_'+ gaparam +'_qiehuan',])

                }
                _that.drawPie(Object.assign(opt, _that.dialogVisible ? opt : {
                  id: 'bar' + opt.index,
                  myMagnify: true
                }));
              }
            ...
            myToggleBar: {
              ...
              onclick() {
                ...
                _that.drawBar(Object.assign(opt, _that.dialogVisible ? opt : {
                  id: 'bar' + opt.index,
                  myMagnify: true
                }))
              }
            ...
            myToggleLine: {
              ...
              onclick() {
                ...
                _that.drawLine(Object.assign(opt, _that.dialogVisible ? opt : {
                  id: 'bar' + opt.index,
                  myMagnify: true
                }));
              }
            ...
            myMagnify: {
              ...
              onclick() {
                _that.handleOpenIndex(Number(/^bar(\d)$/.exec(id)[1]), opt, 'line')
                if(gaparam){
                  window.ga("send", "event", "option", "click", 'cfdadrug_analy_'+ gaparam +'_quanping')
                  window._paq.push(['trackEvent', 'option', 'click', 'cfdadrug_analy_'+ gaparam +'_qiehuan',])
        ...
        toolbox: {
          ...
          feature: {
            ...
            myMagnify: {
              ...
              icon: "image:///static/imgs/echarts/fullscreen.png",
              onclick() {
                _that.handleOpenIndex(Number(/^bar(\d)$/.exec(id)[1]), opt, 'pie')
                if(gaparam){
                  window.ga("send", "event", "option", "click", 'cfdadrug_analy_'+ gaparam +'_quanping')
                  window._paq.push(['trackEvent', 'option', 'click', 'cfdadrug_analy_'+ gaparam +'_qiehuan',])
    ...
    drawBar({
      ...
      chart.setOption({
        ...
        tooltip: data1.length ? {
          ...
          trigger: "item",
          formatter: totaldata.length ? function(v,d) {
            let l = totaldata.length - 1 - v.dataIndex
            return v.name + '<br />'+ `${totaldata[l].cname ? totaldata[l].cname + '<br />' : ''}` +'数量: ' + v.value
          } :'{b}<br />数量: {c}'
        },
        grid: {
        ...
        toolbox: {
          ...
          feature: {
            ...
            myMagnify: {
              ...
              icon: "image:///static/imgs/echarts/fullscreen.png",
              onclick() {
                _that.handleOpenIndex(Number(/^bar(\d)$/.exec(id)[1]), opt, 'bar')
                if(gaparam){
                  window.ga("send", "event", "option", "click", 'cfdadrug_analy_'+ gaparam +'_quanping')
                  window._paq.push(['trackEvent', 'option', 'click', 'cfdadrug_analy_'+ gaparam +'_qiehuan',])
  ...
  watch: {
    kshRes(val) {
      ...
      if(keylen){
        ...
        if(keylen > 1){
          if(this.kshRes[this.titles[keylen - 2].key]&&this.kshRes[this.titles[keylen - 2].key].length){
    ...
    $route:{
      handler(){
        if(this.$route.path==='/fdadrugnew/analy'&&Object.keys(this.kshRes).length<=1){
          Store.dispatch("Fdadrugnew/getKshRes")
        }
        if(this.$route.path==='/fdadrugnew/list.vue'){
          ...
          this.noData && Store.dispatch('Fdadrugnew/nomalSearch',{queryToPutong: false,tabs:true})
        }
