...
let obj = ['/updatelog', '/introduce', '/service'];
...
function loginWWW(_this, callback) { //老版项目登录后
    ...
    Axios.post('/api/synclogin', {
        ...
        if (_data.code == 200) {
            ...
            _window.vueSetCookie('accesstoken', accessToken)
            _window.vueSetCookie('userPicture', _data.data.picture)
            _window.vueSetCookie('userName', _data.data.username)
