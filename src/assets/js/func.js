...
func.install = function(Vue, options) {
  ...
  Vue.prototype.vueHandleInitPutong = function(options) {
    ...
    let _that = this;
    let inpArr = [];
    if (_.keys(_that.putong).length) {
      // 不为空说明有路由参数name
      for (let item of options) {
        let ins = Object.assign({}, item);
        for (let key in _that.putong) {
          if (key.indexOf(item.name) == 0 && !item.value) {
            ins.value = _that.putong[key];
  ...
  Vue.prototype.handleSearch = function(option, vuex_name, storeState) {
    ...
    if (_.keys(obj_putong).length > 0) {
      ...
      _.map(obj_putong, (value, key) => {
        ...
        let findObj = nomal_filed ? _.find(nomal_filed, { name: name }) : {};
        let obj = {
          name: name,
          accurate: accurate,
          value: value,
    ...
    if (_.keys(obj_shaixuan).length > 0) {
      // 处理筛选条件
      let obj_filter = {};
      let arr_shaixuan = _.split(obj_shaixuan.filter_condition, " _and ");
  ...
  Vue.prototype.echartsTooltipMoney = num => {
    ...
    if (state.gaoji && state.gaoji.words) {
      ...
      _.forEach(state.conditions, item => {
        ...
        if (item.value) {
          ...
          gaoji += `${item.logic} ${item.name}${item.accurate ? "_eq" : ""}=${
            type === "object" ? item.value.join("ღ") : item.value
          } `;
        }
      });
      obj = _.assign(obj, { words: gaoji.slice(0, gaoji.length - 1) });
    }
    return obj;
      ...
      query.forEach(item => {
        ...
        if (item.value) {
          ...
          if (key === "searchwords_zh") {
            let wordsObj = _.find(state.advanced_filed, { name: item.name });
            if (wordsObj.type === "select") {
              value = value.map((item, i) => {
                let selectItem = _.find(wordsObj.value, { value: item });
      ...
      for (let key in obj) {
        ...
        if (itemChecked.length > 0) {
          itemChecked.forEach(items => {
            let _obj = _.find(item.data || item, { key: items });
            if (_obj) {
  ...
  (Vue.prototype.keywordStatistics = (params, obj) => {
    ...
    _.forIn(params, (value, key) => {
      ...
      if (typeOf === "string" && value) {
        ...
        let findObj = _.find(item.option, { value: value });
        if (findObj) {
  ...
  Vue.prototype.getExtendList = function(dbname, data) {
    if (!data) return;
    window
      .Axios({ method: "get", url: `/api/${dbname}/extendlistfun?${data}` })
      .then(res => {
        if (res.data.data) {
